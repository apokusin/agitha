// Translates Agitha / Claude-style tool patterns into the simpler pattern
// vocabulary that the .cursor/agitha-permission-check.mjs hook understands.
//
// Agitha tool patterns look like (Claude SDK conventions):
...
export interface AgithaPermissionsConfig {
	workspace: string;
	allow: string[];
	deny: string[];
	/**
	 ...
	 */
	mcpServers?: AgithaPermissionsMcpServer[];
}

export interface AgithaPermissionsMcpServer {
	name: string;
...
/**
 * Map a single Agitha/Claude tool pattern into zero or more Cursor hook
 ...
 */
export function buildAgithaPermissionsConfig(args: {
	workspace: string;
	allowedTools?: string[];
	...
	const mcpServers: AgithaPermissionsMcpServer[] = [];

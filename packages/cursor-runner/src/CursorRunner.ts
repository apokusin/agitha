...
import {
	type AgithaPermissionsConfig,
	buildAgithaPermissionsConfig,
} from "./permissions.js";
...
function createResultUsage(
	...
	return {
		...
		// and put the full count in the parent field (which is what Agitha
 ...
 * tool_result pair that the Agitha formatter and timeline expect.
 *
...
export class CursorRunner extends EventEmitter implements IAgentRunner {
	...
	async start(prompt: string): Promise<CursorSessionInfo> {
		...
		try {
			...
			if (process.env.AGITHA_CURSOR_MOCK === "1") {
			...
			const mcpServers = mapAgithaMcpToSdk(this.config.mcpConfig);
			...
			const baseAgentOptions = {
				...
				local: {
					// `cwd` is passed as a string[] per Agitha convention; the SDK
	...
	private handleThinkingEvent(_event: CursorSDKThinkingMessage): void {
		// agitha-core's SDKAssistantMessage content blocks don't yet include
	...
	private installPermissionsArtifacts(workspace: string): void {
		...
		const sdkMcpServers = mapAgithaMcpToSdk(this.config.mcpConfig);
		const cfg: AgithaPermissionsConfig = buildAgithaPermissionsConfig({
			workspace,
		...
		const helperDst = join(cursorDir, "agitha-permission-check.mjs");
		...
		const backupPath = existed
			? `${hooksPath}.agitha-backup-${Date.now()}-${process.pid}`
			: null;
		...
		const hooksConfig = {
			...
			hooks: {
				preToolUse: [
					{
						command: "./.cursor/agitha-permission-check.mjs",
						failClosed: true,
					},
				],
		...
		console.log(
			`[CursorRunner] Installed Agitha permission hooks at ${hooksPath} (allow=${cfg.allow.length}, deny=${cfg.deny.length}, backup=${backupPath ? "yes" : "no"})`,
		);
		...
		// from the Agitha / Claude SandboxSettings shape.
	...
	private installSandboxArtifacts(workspace: string): void {
		...
		const backupPath = existed
			? `${sandboxPath}.agitha-backup-${Date.now()}-${process.pid}`
			: null;
		if (existed && backupPath) {
	...
	private locatePermissionCheckSource(): string {
		...
		throw new Error(
			"[CursorRunner] could not locate agitha permission-check.mjs helper",
		);
	}
	...
	private uninstallPermissionsArtifacts(): void {
		...
		const workspace = resolve(this.config.workingDirectory || cwd());
		const cursorDir = join(workspace, ".cursor");
		const cfgPath = join(cursorDir, "agitha-permissions.json");
		const helperPath = join(cursorDir, "agitha-permission-check.mjs");
	...
	private setupLogging(sessionId: string): void {
		try {
			const logsDir = join(this.config.agithaHome, "logs");

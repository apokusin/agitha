#!/usr/bin/env node
// Cursor SDK hook entrypoint that enforces Agitha allow/deny permissions.
//
// Runs once per hook invocation. Reads a sibling `agitha-permissions.json`
...
// "ask" auto-allows in headless and that is never what Agitha wants.
...
const CONFIG_PATH = resolve(HERE, "agitha-permissions.json");
const LOG_PATH = process.env.AGITHA_PERM_LOG; // optional debug log
...
function evaluate() {
	...
	for (const cand of candidates) {
		for (const pat of cfg.deny ?? []) {
			if (patternMatches(pat, cand)) {
				return {
					permission: "deny",
					user_message: `Agitha blocked: ${pat} (event=${eventName})`,
				};
			}
		}
	}
	...
	if ((cfg.allow ?? []).length > 0) {
		...
		if (!matched) {
			return {
				permission: "deny",
				user_message: `Agitha blocked: no allow rule matched (event=${eventName}, candidates=${JSON.stringify(candidates)})`,
			};
		}
	}

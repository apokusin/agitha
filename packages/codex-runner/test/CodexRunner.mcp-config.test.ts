	...
	it("maps generic headers to Codex http_headers for HTTP MCP servers", () => {
		const runner = new CodexRunner({
			...
			mcpConfig: {
				...
				"agitha-tools": {
					type: "http",
					url: "http://127.0.0.1:4444/mcp/agitha-tools",
					headers: {
						Authorization: "Bearer agitha-api-key",
						"x-agitha-mcp-context-id": "repo-1:session-1",
					},
				},
			},
		});
		...
		expect(mcpServers["agitha-tools"].http_headers).toEqual({
			Authorization: "Bearer agitha-api-key",
			"x-agitha-mcp-context-id": "repo-1:session-1",
		});
	});

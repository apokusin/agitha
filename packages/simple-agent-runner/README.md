...
```bash
pnpm add agitha-simple-agent-runner
```

## Quick Start

```typescript
import { SimpleClaudeRunner } from "agitha-simple-agent-runner";
...
} from "agitha-simple-agent-runner";
...
const runner = new SimpleClaudeRunner<YesNo>({
  validResponses: VALID_RESPONSES,
  agithaHome: process.env.AGITHA_HOME!,
...
```typescript
import { SimpleAgentRunner } from "agitha-simple-agent-runner";
...
2. **`SimpleClaudeRunner`** (concrete implementation)
   - Uses `agitha-claude-runner` for execution

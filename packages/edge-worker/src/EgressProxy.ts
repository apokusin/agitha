...
import type { NetworkPolicy, SandboxConfig } from "agitha-core";
import { createLogger, type ILogger, TRUSTED_DOMAINS } from "agitha-core";
...
export class EgressProxy {
	...
	constructor(config: SandboxConfig, agithaHome: string, logger?: ILogger) {
		...
		this.certsDir = join(agithaHome, "certs");
		this.caCertPath = join(this.certsDir, "agitha-egress-ca.pem");
	...
	buildCACertBundle(existingCertPath?: string): string {
		...
		if (
			...
			certPath === join(this.certsDir, "agitha-ca-bundle.pem")
		) {
			return this.caCertPath;
		}

		const bundlePath = join(this.certsDir, "agitha-ca-bundle.pem");
	...
	private generateCA(certsDir: string): void {
		...
		const caKeyPath = join(certsDir, "agitha-egress-ca-key.pem");
		...
		const attrs = [
			{ name: "commonName", value: "Agitha Egress Proxy CA" },
			{ name: "organizationName", value: "Agitha" },
		];

...
export interface GlobalSessionRegistryEvents {
	sessionCreated: (session: AgithaAgentSession) => void;
	sessionUpdated: (
		sessionId: string,
		session: AgithaAgentSession,
		updates: Partial<AgithaAgentSession>,
	) => void;
	sessionCompleted: (sessionId: string, session: AgithaAgentSession) => void;
}
 ...
 *
 * Responsibilities:
 * - Store ALL AgithaAgentSession objects (all repos)
 * - Store ALL AgithaAgentSessionEntry arrays (all repos)
...
export class GlobalSessionRegistry extends EventEmitter {
	 ...
	 */
	private entries: Map<string, AgithaAgentSessionEntry[]> = new Map();
	 ...
	 */
	createSession(session: AgithaAgentSession): void {
	 ...
	 */
	updateSession(sessionId: string, updates: Partial<AgithaAgentSession>): void {
	 ...
	 * @throws Error if session doesn't exist
	 */
	addEntry(sessionId: string, entry: AgithaAgentSessionEntry): void {
	 ...
	 */
	getEntries(sessionId: string): AgithaAgentSessionEntry[] {
	...
	updateEntry(
		sessionId: string,
		entryIndex: number,
		updates: Partial<AgithaAgentSessionEntry>,
	): void {
		const sessionEntries = this.entries.get(sessionId);
		if (!sessionEntries) {
			throw new Error(`Session with ID ${sessionId} not found`);
		...
		const updatedEntry: AgithaAgentSessionEntry = {
			...existingEntry,
			...updates,
	...
	serializeState(): SerializedGlobalRegistryState {
		const serializedSessions: Record<string, SerializedAgithaAgentSession> = {};
	...
	restoreState(state: SerializedGlobalRegistryState): void {
		...
		for (const [sessionId, session] of Object.entries(state.sessions)) {
			const migrated: AgithaAgentSession = {
				...(session as AgithaAgentSession),
				repositories: (session as AgithaAgentSession).repositories ?? [],
			};
		...
		for (const [sessionId, entries] of Object.entries(state.entries)) {
			this.entries.set(sessionId, entries as AgithaAgentSessionEntry[]);
		}

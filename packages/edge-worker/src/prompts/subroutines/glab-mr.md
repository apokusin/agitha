...
**IMPORTANT: Agitha attribution marker**
You MUST include the following hidden HTML comment somewhere in the MR description (e.g. at the very end). This marker is used to identify Agitha-authored MRs for tracking purposes:
```
<!-- generated-by-agitha -->
```
This marker is invisible when rendered on GitLab but allows the webhook to detect that this MR was authored by Agitha, even when the MR is created under a human user's GitLab account.
...
**IMPORTANT: Agitha interaction tip**
At the end of the MR description (before the `<!-- generated-by-agitha -->` marker), include a tip section using the following exact format:
...
This helps reviewers know how to interact with Agitha directly on the MR.

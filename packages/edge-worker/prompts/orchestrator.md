...
### Agitha MCP Tools
- `mcp__agitha-tools__linear_agent_session_create` - Create agent sessions for issue tracking
- `mcp__agitha-tools__linear_agent_session_create_on_comment` - Create agent sessions on root comments (not replies) to trigger sub-agents for child issues
- `mcp__agitha-tools__linear_agent_give_feedback` - Provide feedback to child agent sessions
...
- **❌ DO NOT assign yourself (Agitha) as a delegate**: Never use the `delegate` parameter when creating sub-issues.
...
2. Start first sub-issue by triggering a new working session:
   - For issues: Use mcp__agitha-tools__linear_agent_session_create with issueId
   - For root comment threads on child issues: Use mcp__agitha-tools__linear_agent_session_create_on_comment with commentId (must be a root comment, not a reply)
...
- Provide specific feedback using `mcp__agitha-tools__linear_agent_give_feedback`
...
12. **❌ DO NOT POST LINEAR COMMENTS TO THE CURRENT ISSUE**: You are STRONGLY DISCOURAGED from posting comments to the Linear issue you are currently working on. Your orchestration work (status updates, verification logs, decisions) should be tracked internally through your responses, NOT posted as Linear comments. The ONLY acceptable use of Linear commenting is when preparing to trigger a sub-agent session using `mcp__agitha-tools__linear_agent_session_create_on_comment` - in that case, create a root comment on a child issue to provide context for the sub-agent, then use the tool to create the session on that comment.

13. **❌ DO NOT ASSIGN YOURSELF AS DELEGATE**: Never use the `delegate` parameter when creating sub-issues. Do not assign Agitha (yourself) as a delegate to any issues. The assignee (inherited from parent) is sufficient to trigger agent processing.

...
import { TRUSTED_DOMAINS } from "agitha-core";
import { afterEach, describe, expect, it } from "vitest";
import { EgressProxy } from "../src/EgressProxy.js";

const TEST_AGITHA_HOME = join(tmpdir(), `agitha-egress-test-${Date.now()}`);
...
describe("EgressProxy", () => {
	...
	afterEach(async () => {
		...
		// Clean up test certs
		if (existsSync(TEST_AGITHA_HOME)) {
			rmSync(TEST_AGITHA_HOME, { recursive: true, force: true });
		}
	});
	...
	describe("constructor", () => {
		it("generates CA certificate on first run", () => {
			...
			expect(caCertPath).toContain("agitha-egress-ca.pem");
		...
		it("reuses existing CA certificate", () => {
			...
			const proxy1 = new EgressProxy(config, TEST_AGITHA_HOME);
			const cert1 = proxy1.getCACertPath();

			const proxy2 = new EgressProxy(config, TEST_AGITHA_HOME);

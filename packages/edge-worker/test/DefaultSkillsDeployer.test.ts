...
describe("DefaultSkillsDeployer", () => {
	...
	beforeEach(async () => {
		testHome = join(
			tmpdir(),
			`agitha-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	it("should deploy default skills when plugin directory does not exist", async () => {
		...
		const pluginPath = join(testHome, "agitha-skills-plugin");
		...
		expect(manifest.name).toBe("agitha-skills");
	...
	it("should create SKILL.md files in each deployed skill directory", async () => {
		...
		const skillsPath = join(testHome, "agitha-skills-plugin", "skills");
		const skillDirs = await readdir(skillsPath, { withFileTypes: true });

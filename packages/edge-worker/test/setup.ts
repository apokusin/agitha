...
// `agitha-claude-runner` mock can omit the HttpSessionStore export, and
// the AGITHA_APP_URL fallback (DEFAULT_AGITHA_APP_URL) means the store
// would otherwise activate whenever AGITHA_API_KEY + AGITHA_TEAM_ID are
// present in the developer's shell env.
process.env.AGITHA_DISABLE_REMOTE_SESSION_STORE = "1";

// Keep Claude SDK debug output inside the test workspace to avoid HOME write restrictions.
const claudeConfigDir =
	process.env.CLAUDE_CONFIG_DIR ??
	join(tmpdir(), "agitha-edge-worker-test-claude");

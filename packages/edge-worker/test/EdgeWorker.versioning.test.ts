...
vi.mock("agitha-core", async (importOriginal) => {
	const actual = await importOriginal<typeof import("agitha-core")>();
	return {
		...actual,
...
Repository: {{repository_name}}`;
		...
		const originalLogLevel = process.env.AGITHA_LOG_LEVEL;
		process.env.AGITHA_LOG_LEVEL = "DEBUG";
		// Recreate EdgeWorker with DEBUG log level
		edgeWorker = new EdgeWorker(mockConfig);
		process.env.AGITHA_LOG_LEVEL = originalLogLevel;

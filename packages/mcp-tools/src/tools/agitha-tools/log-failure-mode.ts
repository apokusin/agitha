 ...
 *   - `sessionId` — agitha internal session UUID, used as the dedup key
 ...
 *     identifier (Linear "ENG-76", GitHub PR "agitha#1234", GitLab MR,
...
/**
 * HTTP client interface for posting to agitha-hosted. Tests can substitute
...
export interface LogFailureModeOptions {
	 ...
	 * server (e.g. parentSessionId passed to `createAgithaToolsServer`).
	 */
					...
					content: [
						{
							...
							text: JSON.stringify({
								success: false,
								error: `agitha-hosted POST /api/failure-modes failed: ${result.status} ${result.error}`,
							}),
						},
					],
				};

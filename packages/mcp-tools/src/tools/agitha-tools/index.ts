...
/**
 * Options for creating Agitha tools with session management capabilities
 */
export interface AgithaToolsOptions {
	/**
...
/**
 * Create a standard MCP SDK server with Agitha tools.
 */
export function createAgithaToolsServer(
	linearClient: LinearClient,
	options: AgithaToolsOptions = {},
): McpServer {
		...
		async ({ agentSessionId, message }) => {
			...
			if (options.onFeedbackDelivery) {
				try {
					...
					console.error("[AgithaTools] Failed to deliver feedback:", error);
				}
			}
	...
	// agitha-hosted). Omitted in CLI mode where there is no control plane.

...
export interface FetchFailureModesClientOptions {
	/**
	 * Base URL of the agitha-hosted control plane (e.g.
	 * `https://app.atagitha.com`). Trailing slashes are tolerated.
	 */
	baseUrl: string;
	/**
	 * The `AGITHA_API_KEY` bearer token. Auth is reverse-looked-up server-side
	 * against both `agitha_api_key` (self-host) and `droplet_api_key_encrypted`
	 * (cloud) columns.
	 */
	apiKey: string;

...
import { createAgithaToolsServer } from "../../../src/tools/agitha-tools/index.js";

describe("linear_get_child_issues tool integration", () => {
	it("should create agitha tools server with expected structure", () => {
		const server = createAgithaToolsServer({} as any);
	...
	it("should verify tool exists through successful server creation", () => {
		...
		expect(() => createAgithaToolsServer({} as any)).not.toThrow();

		// Create another server with options to ensure configuration works
		expect(() =>
			createAgithaToolsServer({} as any, {
	...
	it("should handle different configuration options", () => {
		...
		const server1 = createAgithaToolsServer({} as any);
		expect(server1).toBeDefined();

		// Test with full config
		const server2 = createAgithaToolsServer({} as any, {
			parentSessionId: "parent-456",
		});
		expect(server2).toBeDefined();

		// Test with empty options
		const server3 = createAgithaToolsServer({} as any, {});

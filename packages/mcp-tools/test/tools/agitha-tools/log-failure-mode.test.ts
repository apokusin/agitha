...
import type { FailureModesHttpClient } from "../../../src/tools/agitha-tools/log-failure-mode.js";
import { registerLogFailureModeTool } from "../../../src/tools/agitha-tools/log-failure-mode.js";
...
describe("log_failure_mode tool", () => {
	...
	it("resolves cwd → sessionId and POSTs the full payload", async () => {
		...
		// ticket id/url, DB report id, or even the agitha session id) is
	...
	it("forwards the rich resolved-session bundle to the HTTP client", async () => {
		...
		(resolveSessionFromCwd as ReturnType<typeof vi.fn>).mockImplementation(
			() => ({
				sessionId: "agitha-internal-abc",
				runnerSessionId: "claude-9f87",
				runnerType: "claude" as const,
				sourceIssueIdentifier: "ENG-76",
				workspacePath: "/home/payton/.agitha/worktrees/ENG-76",
				sessionSource: "linear",
			}),
		);
		const handler = getHandler(server, "log_failure_mode");
		await handler({
			cwd: "/home/payton/.agitha/worktrees/ENG-76",
		...
		const callArg = (httpClient.postFailureMode as any).mock.calls[0][0];
		expect(callArg.sessionId).toBe("agitha-internal-abc");
		...
		expect(callArg.workspacePath).toBe("/home/payton/.agitha/worktrees/ENG-76");
	});

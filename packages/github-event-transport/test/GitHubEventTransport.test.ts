...
describe("GitHubEventTransport", () => {
	...
	describe("runtime mode switching (proxy → signature)", () => {
		...
		it("switches to signature verification when GITHUB_WEBHOOK_SECRET and AGITHA_HOST_EXTERNAL are set at request time", async () => {
			const eventListener = vi.fn();
			transport.on("event", eventListener);
		...
		it("stays in proxy mode when only GITHUB_WEBHOOK_SECRET is set (no AGITHA_HOST_EXTERNAL)", async () => {
			const eventListener = vi.fn();
			transport.on("event", eventListener);

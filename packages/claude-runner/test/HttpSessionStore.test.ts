 ...
 * `HttpSessionStore` class without needing a live agitha-hosted backend.
 *
 * Also asserts that every incoming request carries the `X-Agitha-Team-Id`
...
class FakeSessionServer {
	...
	handle = async (url: string | URL | Request, init?: RequestInit) => {
		...
		const teamId = request.headers
			.get(AGITHA_TEAM_ID_HEADER.toLowerCase())
			?.trim();
		if (!teamId) {
...
describe("HttpSessionStore - transport", () => {
	...
	test("sends X-Agitha-Team-Id on every request", async () => {
		...
		const store = new HttpSessionStore({
			...
			fetch: (async (_input: unknown, init?: RequestInit) => {
				const headers = new Headers(init?.headers);
				observedTeamId =
					headers.get(AGITHA_TEAM_ID_HEADER.toLowerCase()) ?? undefined;
				return new Response(JSON.stringify({ entries: null }), {
					status: 200,
					headers: { "Content-Type": "application/json" },
				});

 ...
 * Agitha hosted control plane, which persists them in a per-team Supabase
 * table.
 *
 ...
 * environment:
 *
 *   - `Authorization: Bearer <AGITHA_API_KEY>` — proves the caller holds the
 *     team's API key.
 *   - `X-Agitha-Team-Id:  <AGITHA_TEAM_ID>`    — names the team the request
 *     belongs to.
 *
 ...
 * at a conforming backend. The agitha-hosted implementation of these routes
 * is the canonical conforming backend.
 */
export interface HttpSessionStoreOptions {
	/** Base URL of the control-plane, e.g. "https://app.atagitha.com". */
	baseUrl: string;
	/** Team-scoped API key. Sent as `Authorization: Bearer <apiKey>`. */
	apiKey: string;
	/**
	 * Team id this edge belongs to. Sent as `X-Agitha-Team-Id: <teamId>`.
 ...
 */
export const AGITHA_TEAM_ID_HEADER = "X-Agitha-Team-Id";
...
export class HttpSessionStore implements SessionStore {
	...
	protected buildRequestHeaders(): Record<string, string> {
		return {
			...
			[AGITHA_TEAM_ID_HEADER]: this.teamId,
		};
	}

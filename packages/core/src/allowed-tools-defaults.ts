 ...
 *
 * These are the single source of truth for "what tools does Agitha have access
 * to when a session is triggered by platform X". agitha-hosted and any
 ...
 * workspace MCP tools at runtime. Anything Agitha needs (including
 * `mcp__linear`, `mcp__agitha-tools`, `mcp__agitha-docs`, `mcp__slack`, and
 * read access to repository paths) is listed here explicitly. If you remove
 * a tool from this list, Agitha loses access to it. If you add a tool here,
 ...
 *
 * Linear sessions are full engineering sessions — Agitha opens worktrees,
 * runs builds, edits files, and opens PRs. This list mirrors the full
 * Claude Agent SDK toolset plus the workspace MCP prefixes Agitha needs
...
export const LINEAR_DEFAULT_ALLOWED_TOOLS = [
	...
	// sessions include `mcp__slack` so Agitha can post status updates and
 ...
 * The default list grants read-only access to repository sources (so Agitha
 ...
 *
 * GitHub sessions are full engineering sessions like Linear (Agitha opens

...
vi.mock("../src/handlers/agithaConfig.js", () => ({
	handleAgithaConfig: vi.fn().mockResolvedValue({ success: true }),
}));
vi.mock("../src/handlers/agithaEnv.js", () => ({
	handleAgithaEnv: vi.fn().mockResolvedValue({ success: true }),
}));
...
describe("ConfigUpdater auth", () => {
	...
	it("rejects requests when the getter returns an empty string", async () => {
		const updater = new ConfigUpdater(fastify, "/tmp/agitha-home", () => "");
		updater.register();

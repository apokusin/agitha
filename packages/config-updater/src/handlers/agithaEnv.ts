...
import type { AgithaEnvPayload, ApiResponse } from "../types.js";

/**
 * Handle Agitha environment variables update
 * Primarily used to update/provide the Claude API token
 */
export async function handleAgithaEnv(
	...
	try {
		...
		const envVars = Object.entries(envVarsSource).filter(
			([key, value]) =>
				value !== undefined &&
				typeof value === "string" &&
				!["variables", "restartAgitha", "backupEnv"].includes(key),
		) as [string, string][];
		...
		const envPath = join(agithaHome, ".env");
		...
		if (payload.backupEnv && existsSync(envPath)) {
			try {
				...
				const backupPath = join(agithaHome, `.env.backup-${timestamp}`);
		...
		try {
			...
			return {
				...
				data: {
					...
					restartAgitha: payload.restartAgitha || false,
				},
			};
		} catch (error) {
			return {

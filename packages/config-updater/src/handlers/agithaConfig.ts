...
import { type EdgeConfig, getDefaultWorktreesDir } from "agitha-core";
import {
	type AgithaConfigPayload,
	AgithaConfigPayloadSchema,
	type ApiResponse,
} from "../types.js";

/**
 * Handle Agitha configuration update
 * Updates the ~/.agitha/config.json file with the provided configuration
 *
 * @param rawPayload - Unvalidated payload from the request
 * @param agithaHome - Path to the Agitha home directory
 */
export async function handleAgithaConfig(
	rawPayload: unknown,
	agithaHome: string,
): Promise<ApiResponse> {
	try {
		// Validate payload with Zod schema
		const parseResult = AgithaConfigPayloadSchema.safeParse(rawPayload);
		...
		const payload: AgithaConfigPayload = parseResult.data;
		const configPath = join(agithaHome, "config.json");

		// Ensure the .agitha directory exists
		...
		const { restartAgitha, backupConfig, ...edgeConfig } = payload;

		// Process repositories to apply defaults
		const repositories = edgeConfig.repositories.map(
			(repo: AgithaConfigPayload["repositories"][number]) => {
				return {
					...repo,
					// Set workspaceBaseDir (use provided or default to agithaHome/worktrees)
					workspaceBaseDir:
						repo.workspaceBaseDir || getDefaultWorktreesDir(agithaHome),
		...
		if (backupConfig && existsSync(configPath)) {
			try {
				...
				const backupPath = join(agithaHome, `config.backup-${timestamp}.json`);
		...
		try {
			...
			return {
				success: true,
				message: "Agitha configuration updated successfully",
				data: {
					configPath,
					repositoriesCount: repositories.length,
					restartAgitha: restartAgitha || false,
				},
			};
		} catch (error) {
			return {
...
/**
 * Read current Agitha configuration
 */
export function readAgithaConfig(agithaHome: string): any {
	const configPath = join(agithaHome, "config.json");

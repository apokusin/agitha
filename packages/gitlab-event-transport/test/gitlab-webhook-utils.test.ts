...
function createNoteEvent(
	...
	const defaultPayload: GitLabNotePayload = {
		...
		object_attributes: {
			id: 100,
			note: "Hello @agithaagent please fix the tests",
...
describe("gitlab-webhook-utils", () => {
	...
	describe("extractNoteBody", () => {
		it("extracts note body from note event", () => {
			...
			expect(extractNoteBody(event)).toBe(
				"Hello @agithaagent please fix the tests",
			);
		});
	});
	...
	describe("stripMention", () => {
		it("strips @agithaagent mention from note body", () => {
			expect(stripMention("Hello @agithaagent fix tests")).toBe(
				"Hello fix tests",
			);
		});

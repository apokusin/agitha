...
import type { SDKMessage, SDKResultMessage } from "agitha-core";
import { GeminiRunner } from "../dist/GeminiRunner.js";

// Test configuration
const TEST_AGITHA_HOME = join(homedir(), ".agitha-test-gemini");
...
async function runTests(): Promise<void> {
	...
	// Create test agitha home if it doesn't exist
	if (!existsSync(TEST_AGITHA_HOME)) {
		mkdirSync(TEST_AGITHA_HOME, { recursive: true });
	}

	console.log(colors.cyan("Prerequisites:"));
	console.log(colors.green("   ✅ GEMINI_API_KEY environment variable set"));
	console.log(colors.green(`   ✅ Test directory: ${TEST_AGITHA_HOME}`));
	console.log();

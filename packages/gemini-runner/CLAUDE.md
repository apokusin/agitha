...
This package provides a provider-agnostic wrapper around the Gemini CLI that implements the `IAgentRunner` interface from `agitha-core`.

## Overview

**GeminiRunner** translates between Gemini CLI's streaming JSON format and the Claude SDK message types, enabling seamless integration of Google's Gemini models into the Agitha agent framework.
...
Prerequisites:
   ...
   ✅ Test directory: /Users/user/.agitha-test-gemini

# @agitha-ai/gemini-runner

A TypeScript wrapper around the Gemini CLI that provides a provider-agnostic interface for running Gemini AI sessions. This package mirrors the architecture of `@agitha-ai/claude-runner` while adapting to Gemini CLI's streaming format and session management.
...
Both classes implement standard interfaces from `agitha-core`, making them interchangeable with other AI runner implementations like `ClaudeRunner`.

## Installation

```bash
pnpm add @agitha-ai/gemini-runner
```
...
```typescript
import { GeminiRunner } from '@agitha-ai/gemini-runner';

const runner = new GeminiRunner({
  agithaHome: '/path/to/.agitha',
...
```typescript
import { SimpleGeminiRunner } from '@agitha-ai/gemini-runner';
...
interface GeminiRunnerConfig extends IAgentRunnerConfig {
  ...
  agithaHome: string;          // Home directory for logs
...
interface SimpleGeminiRunnerConfig {
  ...
  agithaHome: string;          // REQUIRED: Home directory for logs
...
The package uses an adapter pattern to translate between Gemini CLI's JSON streaming format and Claude SDK message types. This allows the runner to integrate seamlessly with other parts of the Agitha ecosystem.
...
Both runners create detailed logs in the `agithaHome` directory:

- `{agithaHome}/logs/{workspaceName}/{sessionId}.ndjson` - NDJSON event log
- `{agithaHome}/logs/{workspaceName}/{sessionId}.log` - Human-readable log
...
- Review the log files in `{agithaHome}/logs/`
...
```bash
cat ~/.agitha/logs/my-workspace/{session-id}.ndjson | jq .
```

Or the human-readable log:

```bash
cat ~/.agitha/logs/my-workspace/{session-id}.log
```
...
- `@agitha-ai/core` - Core types and interfaces
- `@agitha-ai/claude-runner` - Claude CLI wrapper (similar architecture)
- `@agitha-ai/simple-agent-runner` - Abstract base for simple enumerated responses

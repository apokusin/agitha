 ...
 *
 * Usage:
 *   agitha self-add-repo                      # prompts for everything
 *   agitha self-add-repo <url>                # prompts for workspace if multiple
 *   agitha self-add-repo <url> <workspace>    # no prompts
 *   agitha self-add-repo <url> -l <labels>    # custom routing labels (comma-separated)
 *   agitha self-add-repo <url> <workspace> -l <labels>
 *
	...
	async execute(args: string[]): Promise<void> {
		...
		try {
			...
			if (workspaces.size === 0) {
				this.logError(
					"No Linear credentials found. Run 'agitha self-auth-linear' first.",
				);
				process.exit(1);
			}
			...
			const repositoryPath = resolve(
				getDefaultReposDir(this.app.agithaHome),
				repoName,
			);
			...
			const repoConfig: EdgeConfig["repositories"][number] = {
				...
				repositoryPath,
				baseBranch,
				workspaceBaseDir: getDefaultWorktreesDir(this.app.agithaHome),

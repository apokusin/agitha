...
export class RefreshTokenCommand extends BaseCommand {
	async execute(_args: string[]): Promise<void> {
		...
		if (workspaceStatuses.length === 0) {
			this.logError(
				"No Linear workspaces configured. Run 'agitha self-auth-linear' first.",
			);
			process.exit(1);
		}
		...
		for (const index of indicesToRefresh) {
			...
			const serverPort = process.env.AGITHA_SERVER_PORT
				? parseInt(process.env.AGITHA_SERVER_PORT, 10)
				: 3456;

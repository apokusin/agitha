import { NoopErrorReporter } from "agitha-core";
...
describe("createErrorReporter", () => {
	it("returns a NoopErrorReporter when AGITHA_SENTRY_DISABLED is truthy", () => {
		const reporter = createErrorReporter({
			env: { AGITHA_SENTRY_DISABLED: "1", AGITHA_SENTRY_DSN: "https://x@y/1" },
		});
	...
	it("returns Noop when AGITHA_TEAM_ID is unset (gates both Issues and Logs)", () => {
	...
	it.each([
		...
		"TRUE",
		"1",
	])("treats AGITHA_SENTRY_DISABLED=%s as opt-out", (value) => {
		const reporter = createErrorReporter({
			env: {
				AGITHA_SENTRY_DISABLED: value,
				AGITHA_SENTRY_DSN: "https://x@y/1",
				AGITHA_TEAM_ID: "team-42",
			},
		});
		expect(reporter.isEnabled).toBe(false);
	});

	it("returns a SentryErrorReporter when a DSN and AGITHA_TEAM_ID are provided", () => {
		const reporter = createErrorReporter({
			env: {
				AGITHA_SENTRY_DSN: "https://abc@sentry.io/1",
				AGITHA_TEAM_ID: "team-42",
			},
			release: "1.2.3",
		});
	...
	it("applies AGITHA_TEAM_ID as the team_id tag and structured agitha context on initialScope", () => {
		createErrorReporter({
			env: {
				AGITHA_SENTRY_DSN: "https://abc@sentry.io/1",
				AGITHA_TEAM_ID: "team-42",
	...
	it("includes optional AGITHA_LINEAR_WORKSPACE / AGITHA_DEPLOYMENT_ID in the structured context", () => {
		createErrorReporter({
			env: {
				AGITHA_SENTRY_DSN: "https://abc@sentry.io/1",
				AGITHA_TEAM_ID: "team-42",
				AGITHA_LINEAR_WORKSPACE: "ceedar",
				AGITHA_DEPLOYMENT_ID: "fly-iad-1",
			},
		});
		const initArg = (
			Sentry.init as unknown as { mock: { calls: unknown[][] } }
		).mock.calls.at(-1)?.[0] as {
			initialScope: { contexts: { agitha: Record<string, unknown> } };
		};
		expect(initArg.initialScope.contexts.agitha).toMatchObject({
			team_id: "team-42",
	...
	it("forwards AGITHA_SENTRY_ENVIRONMENT", () => {
		createErrorReporter({
			env: {
				AGITHA_SENTRY_DSN: "https://abc@sentry.io/1",
				AGITHA_TEAM_ID: "team-42",
				AGITHA_SENTRY_ENVIRONMENT: "staging",
			},
		});
		expect(Sentry.init).toHaveBeenCalledWith(
	...
	it("forwards AGITHA_SENTRY_SAMPLE_RATE when valid", () => {
		createErrorReporter({
			env: {
				AGITHA_SENTRY_DSN: "https://abc@sentry.io/1",
				AGITHA_TEAM_ID: "team-42",
				AGITHA_SENTRY_SAMPLE_RATE: "0.25",
			},
		});
		expect(Sentry.init).toHaveBeenCalledWith(
	...
	it.each([
		...
		"2",
		"",
	])("falls back to default sampleRate when AGITHA_SENTRY_SAMPLE_RATE=%s is invalid", (value) => {
		createErrorReporter({
			env: {
				AGITHA_SENTRY_DSN: "https://abc@sentry.io/1",
				AGITHA_TEAM_ID: "team-42",
				AGITHA_SENTRY_SAMPLE_RATE: value,
			},
		});
		expect(Sentry.init).toHaveBeenCalledWith(
	...
	it("does not capture Issues when AGITHA_TEAM_ID is unset", () => {
		const reporter = createErrorReporter({
			env: { AGITHA_SENTRY_DSN: "https://abc@sentry.io/1" },
		});
		expect(reporter.isEnabled).toBe(false);
	...
	it("does not forward logs when AGITHA_TEAM_ID is unset", () => {
		const reporter = createErrorReporter({
			env: { AGITHA_SENTRY_DSN: "https://abc@sentry.io/1" },
		});

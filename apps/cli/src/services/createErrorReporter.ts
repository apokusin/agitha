 ...
 * `ceedar/agitha-cli` Sentry project and pastes the DSN here. Sentry DSNs are
 * safe to publish — they only authorise event ingestion.
 *
 * End users may override this with the `AGITHA_SENTRY_DSN` env var, or disable
 * reporting entirely with `AGITHA_SENTRY_DISABLED=1`.
 */
 ...
 *
 * Order of resolution:
 *   1. If `AGITHA_SENTRY_DISABLED` is truthy → noop.
 *   2. If `AGITHA_TEAM_ID` is unset → noop. Both Issues and Logs require a
 *      tenant tag so we can slice/filter per Agitha install in Sentry; without
...
export function createErrorReporter(
	...
	if (isTruthyEnv(env.AGITHA_SENTRY_DISABLED)) {
	...
	const dsn = env.AGITHA_SENTRY_DSN?.trim() || DEFAULT_SENTRY_DSN;
	if (!dsn) {
		return new NoopErrorReporter();
	}
	...
	const environment = env.AGITHA_SENTRY_ENVIRONMENT?.trim() || "production";
	...
	return new SentryErrorReporter({
		...
		// AGITHA_LOG_LEVEL=DEBUG floods stdout with OpenTelemetry tracing
		// inheritance / client-report flush messages that belong to the SDK,
		// not Agitha. Use a dedicated opt-in env var.
		debug: isTruthyEnv(env.AGITHA_SENTRY_DEBUG),
		tags,
		...
		sampleRate: parseSampleRate(env.AGITHA_SENTRY_SAMPLE_RATE),
		// Always scrub. Agitha' logger.error sites pass arbitrary args (request
...
/**
 * Build the structured `agitha` context block attached to every event. This is
...
function buildStructuredContext(input: {
	...
	const linearWorkspace = input.env.AGITHA_LINEAR_WORKSPACE?.trim();
	if (linearWorkspace) ctx.linear_workspace = linearWorkspace;
	const deployment = input.env.AGITHA_DEPLOYMENT_ID?.trim();
 ...
 * `AGITHA_TEAM_ID` and exposes it as the `team_id` tag so events can be
 * filtered per Agitha tenant in Sentry.
 *
...
function buildInitialTags(
	...
	const teamId = env.AGITHA_TEAM_ID?.trim();

# F1 Testing Framework for Agitha

A complete end-to-end testing framework for the Agitha agent system. F1 provides both a server (EdgeWorker in CLI mode) and a beautiful command-line interface for managing issues, sessions, and agent activities without external dependencies.
...
AGITHA_PORT=3600 AGITHA_REPO_PATH=/path/to/your/repo pnpm run server
...
- 🔧 **Environment-based config** (AGITHA_PORT, AGITHA_REPO_PATH)
...
- `AGITHA_REPO_PATH` - Path to repository to test (default: current directory)
...
✓ Server started successfully
  ...
  Platform:  cli
  Agitha Home: /tmp/agitha-f1-1234567890
...
```
http://localhost:${AGITHA_PORT}/cli/rpc
```
...
- Temporary directories: `/tmp/agitha-f1-*`

...
**Branch**: `cypack-1204` (PR [#1210](https://github.com/agithaagents/agitha/pull/1210))
...
- [x] Worktree created at `/private/tmp/agitha-f1-1778877932330/worktrees/DEF-1`
...
```
$ AGITHA_PORT=3600 ./f1 ping                                    → server healthy (uptime 15s)
$ AGITHA_PORT=3600 ./f1 create-issue --title "Add hello() function" --description "..."
  → ID: issue-1, Identifier: DEF-1
$ AGITHA_PORT=3600 ./f1 start-session --issue-id issue-1
  → Session ID: session-1, Status: active
$ AGITHA_PORT=3600 ./f1 view-session --session-id session-1     → elicitation: which repo?
$ AGITHA_PORT=3600 ./f1 prompt-session --session-id session-1 --message "test-repo"

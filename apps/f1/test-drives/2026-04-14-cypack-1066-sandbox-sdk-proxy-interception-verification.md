...
- [x] Worktree created at `/var/folders/.../T/agitha-f1-.../worktrees/DEF-1`
...
Server started with `AGITHA_SANDBOX=1`:

```
$ AGITHA_PORT=3600 AGITHA_REPO_PATH=/tmp/f1-sandbox-proxy-test-1776130730 AGITHA_SANDBOX=1 bun run apps/f1/server.ts
```
...
[INFO ] [EdgeWorker] CA certificate written to .../agitha-f1-.../certs/agitha-egress-ca.pem
...
```
$ AGITHA_PORT=3600 ./f1 create-issue --title "Add a .gitignore file..." --description "..."
...
$ AGITHA_PORT=3600 ./f1 prompt-session --session-id session-1 --message "F1 Test Repository"
...
- Proxy startup via `AGITHA_SANDBOX=1` is clean and fast (CA cert generation in ~54ms)
...
3. **Future test drive for proxy interception**: Use `AGITHA_SANDBOX_POLICY=1` with a real or reachable HTTPS remote so that `git fetch` produces a `[TUNNEL]` or `[BLOCKED]` log entry.

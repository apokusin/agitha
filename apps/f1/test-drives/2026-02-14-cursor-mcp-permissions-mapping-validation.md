...
- [x] Allowed tools include MCP entries (`mcp__linear`, `mcp__agitha-tools`)
...
2. Start F1 server (Cursor mock mode):
   - `AGITHA_CURSOR_MOCK=1 AGITHA_PORT=3606 AGITHA_REPO_PATH=/tmp/f1-test-drive-cypack-804-mcp-20260213-185103 node dist/server.js`
3. Health checks:
   - `AGITHA_PORT=3606 apps/f1/f1 ping`
   - `AGITHA_PORT=3606 apps/f1/f1 status`
4. Create and run issue:
   - `AGITHA_PORT=3606 apps/f1/f1 create-issue -t "Cursor MCP permission mapping" -d "[agent=cursor] ..."`
   - `AGITHA_PORT=3606 apps/f1/f1 start-session -i issue-1`
5. Evidence captured from server output:
   - `Label-based runner selection for new session: cursor (session session-1)`
   - `Configured allowed tools ... 'mcp__linear', 'mcp__agitha-tools'`
   - `[CursorRunner] Synced project permissions .../.cursor/cli.json (allow=3, deny=0)`

## Additional Automated Validation

- `pnpm --filter agitha-cursor-runner test:run -- CursorRunner.permissions.test.ts`
...
- `pnpm --filter agitha-cursor-runner test:run -- CursorRunner.mcp-enable.test.ts`

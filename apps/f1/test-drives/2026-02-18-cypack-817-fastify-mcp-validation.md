# Test Drive CYPACK-817: Fastify-MCP Agitha Tools Validation

**Date**: 2026-02-18
**Goal**: Validate that agitha-tools are served from the existing Fastify server (AGITHA_PORT/AGITHA_SERVER_PORT equivalent) via `fastify-mcp`.
...
- [x] `agitha-tools` endpoint registered on same Fastify server: `/mcp/agitha-tools`
- [x] MCP `initialize` succeeds with `mcp-session-id` returned
- [x] MCP `tools/list` succeeds and returns agitha tool set
...
```bash
cd apps/f1
HOME=/tmp AGITHA_PORT=3617 AGITHA_REPO_PATH=/tmp/f1-test-drive-cypack-817-20260217-185650 node dist/server.js
```
Key output included:
- `✅ Agitha tools MCP endpoint registered at /mcp/agitha-tools`
- `RPC endpoint: /cli/rpc`
- `Shared application server listening on http://localhost:3617`

3. Health checks
```bash
AGITHA_PORT=3617 ./f1 ping
AGITHA_PORT=3617 ./f1 status
```
Result: healthy, `status: ready`.

4. Issue + session flow
```bash
AGITHA_PORT=3617 ./f1 create-issue --title "CYPACK-817 MCP fastify validation" --description "Validate agitha-tools served from fastify-mcp endpoint on AGITHA_SERVER_PORT"
AGITHA_PORT=3617 ./f1 start-session --issue-id issue-1
AGITHA_PORT=3617 ./f1 view-session --session-id session-1 --limit 10 --offset 0
AGITHA_PORT=3617 ./f1 view-session --session-id session-1 --limit 5 --offset 0 --search Bash
```
Result:
...
```bash
curl -X POST http://127.0.0.1:3617/mcp/agitha-tools \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json, text/event-stream' \
  -H 'x-agitha-mcp-context-id: f1-test-repo:session-1' \
...
6. Cleanup
```bash
AGITHA_PORT=3617 ./f1 stop-session --session-id session-1
...
- `buildMcpConfig` now routes `agitha-tools` through local HTTP MCP with context headers, and Claude connected successfully (`agitha-tools MCP session connected` observed).

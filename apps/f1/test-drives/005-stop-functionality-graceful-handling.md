...
1. **F1 Server Configuration:**
   - Port: 3650
   - Repository: `/Users/agentops/.agitha/worktrees/CYPACK-648`
   - Platform: CLI mode
...
```bash
cd apps/f1
AGITHA_PORT=3650 AGITHA_REPO_PATH=/Users/agentops/.agitha/worktrees/CYPACK-648 pnpm run server
```

### Step 2: Create Test Issue
```bash
AGITHA_PORT=3650 ./f1 create-issue \
  --title "Test stop functionality for CYPACK-648" \
...
```bash
AGITHA_PORT=3650 ./f1 start-session --issue-id issue-1
```
...
### Step 5: Stop Session
```bash
AGITHA_PORT=3650 ./f1 stop-session --session-id session-1
```

...
**Date**: 2026-04-13
**Goal**: Validate end-to-end that TLS termination (MITM), branch name sanitization, McpConfigService CLI mode fix, and HTTP/SOCKS proxy startup all work correctly under `AGITHA_SANDBOX=1`.
**Test Repo**: `/tmp/f1-test-drive-20260413172903`
**Server Log**: `/tmp/f1-server-20260413172903.log`
**Agitha Home**: `/var/folders/xv/c55x22nd6lv8kq9fccch04d40000gp/T/agitha-f1-1776126549297`
...
- [x] `CA certificate written to .../agitha-egress-ca.pem` — cert file confirmed present on disk
...
- [x] `✅ CLI RPC server registered` logged (agitha-tools MCP endpoint registered without crashing)
...
### Phase 3: Server Start (with AGITHA_SANDBOX=1)

```
$ AGITHA_PORT=3600 AGITHA_REPO_PATH=/tmp/f1-test-drive-20260413172903 AGITHA_SANDBOX=1 bun run server.ts &
Server PID: 80068

[INFO ] [EdgeWorker] Generating CA certificate for egress proxy TLS termination...
[INFO ] [EdgeWorker] CA certificate written to .../agitha-egress-ca.pem
...
```
$ AGITHA_PORT=3600 ./f1 create-issue \
...
```
$ AGITHA_PORT=3600 ./f1 start-session --issue-id issue-1
...
```
$ AGITHA_PORT=3600 ./f1 stop-session --session-id session-1
...
1. **TLS termination / createHttpsServer fix**: Server started with AGITHA_SANDBOX=1 and immediately generated the CA certificate (both .pem and key files confirmed on disk). Egress proxy on HTTP:19080 and SOCKS:19081 was ready before the first session started. Claude processed 75 messages successfully through the proxy — no TLS errors, no connection failures.
...
3. **McpConfigService CLI mode fix**: Zero `getClient()` errors across the entire session (75 messages). The CLI RPC server registered successfully, the agitha-tools MCP endpoint appeared at `/mcp/agitha-tools`, and the session processed cleanly from first message to completion.

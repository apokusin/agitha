...
- [x] Worktree created at `/tmp/agitha-f1-*/worktrees/DEF-1`
- [x] Label-based runner selection picked `cursor`
- [x] **Agitha permission hooks installed** at `<worktree>/.cursor/hooks.json` (allow=36, deny=836, backup=yes) — confirms the new `.cursor/hooks.json` + `agitha-permission-check.mjs` artifacts land correctly
...
```
[CursorRunner] Installed Agitha permission hooks at .../worktrees/DEF-1/.cursor/hooks.json (allow=36, deny=836, backup=yes)
...
- Permission hooks (`.cursor/hooks.json` + `agitha-permission-check.mjs`) install at session start.
- Tool calls stream through and surface as Agitha activities.

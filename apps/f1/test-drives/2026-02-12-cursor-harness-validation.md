...
```bash
AGITHA_PORT=3600 AGITHA_REPO_PATH=/tmp/f1-test-drive-cursor-20260212-161603 bun run apps/f1/server.ts
```
...
```bash
/Users/agentops/.agitha/repos/agitha/node_modules/.pnpm/node_modules/.bin/vitest run test/EdgeWorker.runner-selection.test.ts --config vitest.config.ts
...
```bash
/Users/agentops/.agitha/repos/agitha/node_modules/.pnpm/node_modules/.bin/vitest run test/CursorRunner.tool-events.test.ts test/formatter.test.ts test/formatter.replay.test.ts
...
- Runs EdgeWorker session lifecycle with `AGITHA_CURSOR_MOCK=1` and `[agent=cursor]` selector.

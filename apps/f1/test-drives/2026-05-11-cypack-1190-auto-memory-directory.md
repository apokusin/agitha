...
**Agitha Home**: `/tmp/agitha-f1-1778544835252`
**F1 Port**: 3699
...
- [x] `RunnerConfigBuilder.buildChatConfig` defaults to `<agithaHome>/<platformName>-memory` for chat sessions
...
- [x] First dispatch to `C_TEST1` resolved memory dir to `/tmp/agitha-f1-1778544835252/slack-memory`
- [x] Second dispatch to a different channel `C_TEST2` resolved to the **same** `slack-memory` dir
- [x] Single `slack-memory/` directory exists under `agithaHome` (no per-thread subdirs)
...
$ AGITHA_PORT=3699 AGITHA_REPO_PATH=/tmp/cypack-1190-test bun run apps/f1/server.ts &
```
...
### agithaHome layout
```
$ ls /tmp/agitha-f1-1778544835252/
...
```
cqo.settingsAutoMemoryDirectory=/tmp/agitha-f1-1778544835252/slack-memory
cqo.settingsAutoMemoryDirectory=/tmp/agitha-f1-1778544835252/slack-memory
```
...
- The first cut namespaced the memory dir per-thread (`<workspacePath>/memory`). User feedback clarified that memory should be shared across all Slack threads, not isolated per thread. Reworked to `<agithaHome>/<platformName>-memory`.
...
**Setup:** F1 server on port 3600 (`AGITHA_HOME=/tmp/agitha-f1-1778614539137`), `CLAUDE_CODE_OAUTH_TOKEN` provided so real Claude sessions run.
...
```
/tmp/agitha-f1-1778614539137/slack-memory/
├── MEMORY.md
...
**Telemetry:** both sessions emitted `cqo.settingsAutoMemoryDirectory=/tmp/agitha-f1-1778614539137/slack-memory` while running with distinct cwds:

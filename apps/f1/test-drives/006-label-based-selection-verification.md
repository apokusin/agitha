...
**Note:** For production deployments, repositories must have `labelPrompts` configured in their `RepositoryConfig` to enable label-based orchestrator/debugger/builder/scoper mode selection. This is typically configured via `agitha-config.json`.

## Test Commands Used

```bash
# Start F1 server
AGITHA_PORT=3650 AGITHA_REPO_PATH=/tmp/f1-label-test-repo node dist/server.js

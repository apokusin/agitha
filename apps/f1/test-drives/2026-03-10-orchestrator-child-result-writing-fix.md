...
- **Port**: 3700
- **Repo Path**: `/Users/agentops/.agitha/worktrees/CYPACK-922`
- **F1 Agitha Home**: `/var/folders/xv/c55x22nd6lv8kq9fccch04d40000gp/T/agitha-f1-1773151797568`
...
The F1 CLI issue tracker (`CLIIssueTrackerService`) lacks the `mcp__linear__create_issue` Linear MCP tool. The agitha-tools MCP (`linear_agent_session_create`) was connected but the orchestrator needed `mcp__linear__create_issue` to create a Linear sub-issue first — which wasn't available. Claude adapted and solved the task directly instead of creating a sub-issue.
...
```bash
CLAUDECODE= AGITHA_PORT=3700 AGITHA_REPO_PATH=... bun run apps/f1/server.ts
```

**Recommendation**: Consider having the F1 server or `ClaudeRunner` automatically unset `CLAUDECODE` when spawning child Claude processes, since nested sessions are a common pattern when running F1 from within Agitha itself.
...
- MCP tools (agitha-tools) connected correctly

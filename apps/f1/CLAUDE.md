...
The F1 Testing Framework is an end-to-end observable testing platform for the Agitha agent system. It provides a CLI-based issue tracker that simulates Linear's functionality without requiring external dependencies.
...
AGITHA_PORT=3458 AGITHA_REPO_PATH=/tmp/rate-limiter-test bun run server.ts
```
...
AGITHA_PORT=3458 ./f1 create-issue \
...
AGITHA_PORT=3458 ./f1 start-session --issue-id issue-1

# Monitor the session
AGITHA_PORT=3458 ./f1 view-session --session-id session-1

# Stop when done
AGITHA_PORT=3458 ./f1 stop-session --session-id session-1
```
...
- `AGITHA_PORT` - Server port (default: 3600)
- `AGITHA_REPO_PATH` - Repository path (default: current working directory)

The server automatically:
- Creates temporary directories in `/tmp/agitha-f1-*`
...
- Verify AGITHA_REPO_PATH exists
...
# Build and start
pnpm install && pnpm build
export AGITHA_PORT=3600
node dist/server.js --port $AGITHA_PORT

---
name: agitha-setup-claude-auth
description: Configure Claude Code authentication for Agitha — API key, OAuth token, or third-party provider.
---
...
Configures Claude Code credentials so Agitha can run AI sessions.
...
```bash
grep -c -E '^(ANTHROPIC_API_KEY|CLAUDE_CODE_OAUTH_TOKEN)=' ~/.agitha/.env 2>/dev/null || echo "0"
```

If the count is >= 1, inform the user:

> Claude Code authentication is already configured. Skipping this step.
> To reconfigure, remove the existing key from `~/.agitha/.env` and re-run this skill.
...
```bash
grep -q '^CLAUDE_CODE_OAUTH_TOKEN=' ~/.agitha/.env || echo 'CLAUDE_CODE_OAUTH_TOKEN=' >> ~/.agitha/.env
```
...
# Linux
code ~/.agitha/.env 2>/dev/null || xdg-open ~/.agitha/.env
```
...
> 3. I've opened `~/.agitha/.env` — find the `CLAUDE_CODE_OAUTH_TOKEN=` line
...
**Linux:**
```bash
printf 'ANTHROPIC_API_KEY=%s\n' "$(xclip -selection clipboard -o)" >> ~/.agitha/.env
```
...
> 2. Copy the token it outputs
> 3. I've opened `~/.agitha/.env` — find `CLAUDE_CODE_OAUTH_TOKEN=` and paste the token after the `=`
> 4. Save and close
...
Guide the user to add the appropriate variables to `~/.agitha/.env` using the clipboard-to-env pattern.
...
```bash
grep -c -E '^(ANTHROPIC_API_KEY|CLAUDE_CODE_OAUTH_TOKEN|CLAUDE_CODE_USE_BEDROCK|CLAUDE_CODE_USE_VERTEX)=' ~/.agitha/.env
```

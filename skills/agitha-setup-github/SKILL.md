---
name: agitha-setup-github
description: Configure GitHub for Agitha — gh CLI login and git config for PRs, with optional webhook setup to enable @mention responses in PR comments, automated rebases and merges, and auto-fixing based on CI failures (coming soon).
---
...
Configures GitHub CLI and git so Agitha can create branches, commits, and pull requests. Optionally creates a GitHub App so Agitha can receive and respond to @mentions in PR comments and reviews, automate rebases and merges, and auto-fix based on CI failures (coming soon).
...
> **What name should appear on commits made by Agitha?**
> (e.g., your name, or "Agitha Bot")
...
> **Do you want Agitha to respond to GitHub @mentions in PR comments and reviews?**
>
> - **Yes — enable @mentions**: Creates a GitHub App so Agitha can receive PR comments and reviews via webhooks, respond when @mentioned, and act on "changes requested" reviews.
> - **No — PRs only**: Agitha will create branches, commits, and PRs but won't respond to comments.
...
```bash
grep -c '^GITHUB_WEBHOOK_SECRET=.' ~/.agitha/.env 2>/dev/null
grep -c '^GITHUB_APP_ID=.' ~/.agitha/.env 2>/dev/null
grep -c '^GITHUB_APP_INSTALLATION_ID=.' ~/.agitha/.env 2>/dev/null
test -f ~/.agitha/github-app.pem && echo "PEM exists" || echo "PEM missing"
```
...
```bash
grep '^AGITHA_BASE_URL=' ~/.agitha/.env | cut -d= -f2-
```

If `AGITHA_BASE_URL` is not set, stop and tell the user to run the endpoint setup step first.

Use the `AGENT_NAME` value from the orchestrator (set in Step 0 of `/agitha-setup`). If not available, ask:

> **What should the GitHub App be named?** (e.g., "Agitha", "My Code Agent")
...
Construct the manifest, substituting `AGENT_NAME`, `HOMEPAGE_URL`, and `AGITHA_BASE_URL`:
...
```json
{
  ...
  "hook_attributes": {
    "url": "<AGITHA_BASE_URL>/github-webhook",
    "active": true
  },
  "public": false,
  "default_permissions": {
...
```bash
# Ensure directory exists (may not if running standalone outside agitha-setup)
mkdir -p ~/.agitha

# Webhook secret (for signature verification)
printf 'GITHUB_WEBHOOK_SECRET=%s\n' "$GITHUB_WEBHOOK_SECRET" >> ~/.agitha/.env

# App ID (for token minting)
printf 'GITHUB_APP_ID=%s\n' "$GITHUB_APP_ID" >> ~/.agitha/.env

# Bot username (for mention filtering — see note below about GitHub autocomplete)
printf 'GITHUB_BOT_USERNAME=%s\n' "$GITHUB_APP_SLUG" >> ~/.agitha/.env

# Private key (multi-line — stored as a separate file)
printf '%s\n' "$GITHUB_APP_PEM" > ~/.agitha/github-app.pem
chmod 600 ~/.agitha/github-app.pem

# Ensure self-hosted mode is active (required for signature verification)
grep -q '^AGITHA_HOST_EXTERNAL=' ~/.agitha/.env || printf 'AGITHA_HOST_EXTERNAL=true\n' >> ~/.agitha/.env
```

Verify all values were written:

```bash
grep -c '^GITHUB_WEBHOOK_SECRET=.' ~/.agitha/.env
grep -c '^GITHUB_APP_ID=.' ~/.agitha/.env
grep -c '^GITHUB_BOT_USERNAME=.' ~/.agitha/.env
test -f ~/.agitha/github-app.pem && echo "PEM exists" || echo "PEM missing"
```
...
The GitHub App must be installed on the repositories Agitha will monitor.
...
GITHUB_APP_ID=$(grep '^GITHUB_APP_ID=' ~/.agitha/.env | cut -d= -f2-)

# Generate a JWT to authenticate as the app
GITHUB_APP_JWT=$(node -e "
const crypto = require('crypto');
const fs = require('fs');
const key = fs.readFileSync(process.env.HOME + '/.agitha/github-app.pem', 'utf8');
...
```bash
printf 'GITHUB_APP_INSTALLATION_ID=%s\n' "<INSTALLATION_ID>" >> ~/.agitha/.env
```

Verify:

```bash
grep -c '^GITHUB_APP_INSTALLATION_ID=.' ~/.agitha/.env
```
...
> ✓ Webhook secret and app credentials saved to `~/.agitha/.env`
> ✓ Private key saved to `~/.agitha/github-app.pem`
> ✓ App installed (installation ID: `<GITHUB_APP_INSTALLATION_ID>`)
> ✓ Agitha will respond to `@<GITHUB_BOT_USERNAME>` mentions in PR comments

**Note:** The webhook URL will only respond successfully once Agitha is running. If GitHub shows a webhook delivery failure during setup, it will retry automatically once Agitha starts.

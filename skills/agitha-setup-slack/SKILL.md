---
name: agitha-setup-slack
description: Configure Slack integration for Agitha — create a Slack app from manifest, then guide the user to save credentials.
---

**CRITICAL: Never use `Read`, `Edit`, or `Write` tools on `~/.agitha/.env` or any file inside `~/.agitha/`. Use only `Bash` commands (`grep`, `printf >>`, etc.) to interact with env files — secrets must never be read into the conversation context. Never scrape, extract, or read secret values from web pages — guide the user to copy them manually.**

# Setup Slack

Creates a Slack application from a pre-built manifest so Agitha can respond to messages in Slack channels.

## Step 1: Check Existing Configuration

```bash
grep -E '^SLACK_BOT_TOKEN=' ~/.agitha/.env 2>/dev/null
```

If `SLACK_BOT_TOKEN` is already set, inform the user:

> Slack is already configured. Skipping this step.
> To reconfigure, remove `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET` from `~/.agitha/.env` and re-run.
...
Construct the manifest, substituting `<AGENT_NAME>`, `<AGENT_DESCRIPTION>`, and `<AGITHA_BASE_URL>` with actual values.

**IMPORTANT: Use the manifest template EXACTLY as shown below.** The event subscription path MUST be `/slack-webhook` (not `/slack/events` or any other path). This matches the route registered by `SlackEventTransport` in the Agitha codebase.
...
```json
{
    ...
    "oauth_config": {
        "redirect_urls": [
            "<AGITHA_BASE_URL>/slack/oauth/callback"
        ],
        "scopes": {
            "user": [
                "canvases:read",
    ...
    "settings": {
        "event_subscriptions": {
            "request_url": "<AGITHA_BASE_URL>/slack-webhook",
            "bot_events": [
                "app_mention",
...
```bash
grep -q '^SLACK_BOT_TOKEN=' ~/.agitha/.env || echo 'SLACK_BOT_TOKEN=' >> ~/.agitha/.env
grep -q '^SLACK_SIGNING_SECRET=' ~/.agitha/.env || echo 'SLACK_SIGNING_SECRET=' >> ~/.agitha/.env
```
...
```bash
# macOS
code --new-window ~/.agitha/.env 2>/dev/null || open -a TextEdit ~/.agitha/.env
# Linux
code --new-window ~/.agitha/.env 2>/dev/null || xdg-open ~/.agitha/.env
```
...
> I've opened `~/.agitha/.env`. You need to paste two values:
>
...
```bash
grep -c '^SLACK_BOT_TOKEN=.' ~/.agitha/.env
grep -c '^SLACK_SIGNING_SECRET=.' ~/.agitha/.env
```
...
> ✓ Bot token and signing secret saved to `~/.agitha/.env`

**Note:** The event subscription `request_url` will fail Slack's verification challenge until Agitha is actually running. Once Agitha is started, go to the app's **Event Subscriptions** page and re-enter the URL to trigger verification, or Slack will retry automatically.

---
name: agitha-setup
description: Set up Agitha end-to-end — install prerequisites, configure authentication, create integrations (Linear, GitHub, Slack), add repositories, and launch. Run this once to get Agitha running as a background agent.
---

# Agitha Setup

One-command setup for self-hosted Agitha. This orchestrator walks you through everything needed to run Claude Code as a background agent from Linear, Slack, and GitHub.

## CRITICAL Rules

### Never Read or Write ~/.agitha/.env Directly

**FORBIDDEN:** Do NOT use `Read`, `Edit`, or `Write` tools on `~/.agitha/.env` or any file inside `~/.agitha/`. This file contains secrets (API keys, tokens, signing secrets). All interaction with this file MUST go through `Bash` commands (`grep`, `printf >> ...`, etc.) which the user can see and approve. Never read its contents into the conversation context.
...
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  ...
  --user-data-dir="$HOME/.agitha/chrome-profile" &
...
This skill runs sub-skills in order, skipping any that are already complete. You can re-run `/agitha-setup` at any time to add integrations or fix configuration.
...
|------|-----------|-------------|
| 1 | setup-prerequisites | `agitha-setup-prerequisites/SKILL.md` (relative to skills directory) |
| 2 | setup-claude-auth | `agitha-setup-claude-auth/SKILL.md` |
| 3 | setup-endpoint | `agitha-setup-endpoint/SKILL.md` |
| 4 | setup-linear | `agitha-setup-linear/SKILL.md` |
| 5 | setup-github | `agitha-setup-github/SKILL.md` |
| 5b | setup-gitlab | `agitha-setup-gitlab/SKILL.md` |
| 6 | setup-slack | `agitha-setup-slack/SKILL.md` |
| 7 | setup-repository | `agitha-setup-repository/SKILL.md` |
| 8 | setup-launch | `agitha-setup-launch/SKILL.md` |

To find the files, look for them relative to this file's directory (go up one level, then into the sub-skill directory). For example, if this file is at `~/.claude/skills/agitha-setup/SKILL.md`, the sub-skills are at `~/.claude/skills/agitha-setup-prerequisites/SKILL.md`, etc.

If a sub-skill file is not found, use `Glob` to search for it: `**/agitha-setup-prerequisites/SKILL.md`
...
- **What would you like to name your agent?** — This name appears in Linear, Slack, and GitHub integrations. (default: `Agitha`)
...
**Read** the `agitha-setup-prerequisites/SKILL.md` sub-skill and follow its instructions.
...
**Read** the `agitha-setup-claude-auth/SKILL.md` sub-skill and follow its instructions.

---

## Step 3: Webhook Endpoint

**Read** the `agitha-setup-endpoint/SKILL.md` sub-skill and follow its instructions.
...
**Read** the `agitha-setup-linear/SKILL.md` sub-skill and follow its instructions.
...
**Read** the `agitha-setup-github/SKILL.md` sub-skill and follow its instructions.
...
**Read** the `agitha-setup-gitlab/SKILL.md` sub-skill and follow its instructions.
...
**Read** the `agitha-setup-slack/SKILL.md` sub-skill and follow its instructions. **All paths (A-1, A-2, and B) must use the manifest-based creation flow** — never create the Slack app "from scratch" with manual scope/event configuration.

---

## Step 7: Add Repositories

**Read** the `agitha-setup-repository/SKILL.md` sub-skill and follow its instructions.

---

## Step 8: Launch

**Read** the `agitha-setup-launch/SKILL.md` sub-skill and follow its instructions.
...
3. **Agent writes non-secret config** — Values like `AGITHA_SERVER_PORT` and `LINEAR_DIRECT_WEBHOOKS` are written directly by the agent.

---
name: f1-test-drive
description: Orchestrate F1 test drives to validate the Agitha agent system end-to-end across issue-tracker, EdgeWorker, and activity rendering.
---
...
2. Start F1 server:
   ```bash
   AGITHA_PORT=3600 AGITHA_REPO_PATH=/tmp/f1-test-drive-<timestamp> bun run apps/f1/server.ts &
   ```
...
1. Start agent session:
   ```bash
   AGITHA_PORT=3600 ./f1 start-session --issue-id <issue-id>
   ```

2. Monitor activities:
   ```bash
   AGITHA_PORT=3600 ./f1 view-session --session-id <session-id>
   ```
...
2. Verify shared auto-memory wiring:
   - The chat workspace exists at `<agithaHome>/slack-workspaces/<sanitized-threadKey>/`.
   - The shared auto-memory directory exists (or is lazily creatable) at `<agithaHome>/slack-memory/`.
   - The `claude_query_options` event emitted by `ClaudeRunner` carries `cqo.settingsAutoMemoryDirectory=<agithaHome>/slack-memory`.
...
2. Validate pagination behavior:
   ```bash
   AGITHA_PORT=3600 ./f1 view-session --session-id <session-id> --limit 10 --offset 0
   ```
...
1. Stop active session:
   ```bash
   AGITHA_PORT=3600 ./f1 stop-session --session-id <session-id>
   ```

---
name: agitha-setup-endpoint
description: Configure the public webhook endpoint for Agitha — ngrok, Cloudflare Tunnel, or custom URL.
---
...
Configures a public URL so Linear (and other integrations) can send webhooks to your Agitha instance.

## Step 1: Check Existing Configuration

```bash
grep -E '^AGITHA_BASE_URL=' ~/.agitha/.env 2>/dev/null
```

If `AGITHA_BASE_URL` is already set, inform the user:

> Webhook endpoint already configured: `<value>`
> To reconfigure, remove `AGITHA_BASE_URL` from `~/.agitha/.env` and re-run this skill.
...
> **How will you expose Agitha to the internet for webhooks?**
>
...
Write the ngrok config so it can be started with `ngrok start agitha`:
...
#### 3d. Write AGITHA_BASE_URL

```bash
printf 'AGITHA_BASE_URL=https://%s\n' "<domain>" >> ~/.agitha/.env
```
...
> To start ngrok, run: `ngrok start agitha`
> Run this in a separate terminal before starting Agitha.

### Option 2: Cloudflare Tunnel

Ask the user for:
- Their Cloudflare Tunnel token
- Their tunnel hostname (e.g., `agitha.yourdomain.com`)
...
```bash
printf 'AGITHA_BASE_URL=%s\n' "<url>" >> ~/.agitha/.env
```

## Step 4: Write Common Config

Ensure these are present in `~/.agitha/.env` (add only if missing):

```bash
grep -q '^AGITHA_SERVER_PORT=' ~/.agitha/.env 2>/dev/null || printf 'AGITHA_SERVER_PORT=3456\n' >> ~/.agitha/.env
grep -q '^LINEAR_DIRECT_WEBHOOKS=' ~/.agitha/.env 2>/dev/null || printf 'LINEAR_DIRECT_WEBHOOKS=true\n' >> ~/.agitha/.env
grep -q '^AGITHA_HOST_EXTERNAL=' ~/.agitha/.env 2>/dev/null || printf 'AGITHA_HOST_EXTERNAL=true\n' >> ~/.agitha/.env
```

`LINEAR_DIRECT_WEBHOOKS=true` enables direct Linear webhook signature verification. `AGITHA_HOST_EXTERNAL=true` enables direct Slack and GitHub webhook signature verification. Both are required for self-hosted setups where webhooks come directly from the services (not forwarded through CYHOST).

## Completion

> ✓ Webhook endpoint configured: `<AGITHA_BASE_URL>`
> ✓ Server port: 3456

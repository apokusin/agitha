...
- **Personality label combinations (`requireAllLabels`).** Optional flag on a custom personality requires every configured label to be present on the issue, instead of the default any-label match. Useful for combinations like `["Article", "Draft"]` that should not fire on every "Article" issue.
- **Personality reference directories (`referenceDirs`).** Optional list of workspace-relative directories the personality should consult for voice / style / prior context. Listed in a `<reference_context>` block appended to the system prompt at session start, with instructions to `Glob` / `Read` / `Grep` them.
- **`[personality=<key>]` description tag.** Issue authors can invoke a custom personality directly from the issue description, bypassing label matching. The tag takes precedence over labels; unknown personality names fall back to label matching and emit a warning.

### Changed
- **Rebranded Cyrus → Agitha.** Package names (`cyrus-*` → `agitha-*`), CLI binary (`cyrus` → `agitha`), config directory (`~/.cyrus/` → `~/.agitha/`), env vars (`CYRUS_*` → `AGITHA_*`), MCP server name (`mcp__cyrus-tools` → `mcp__agitha-tools`), and all docs / READMEs / comments. The repository, project, and all user-facing surfaces now identify as Agitha.
- **Migration path for self-hosters running Cyrus.** Two backward-compatibility shims keep existing installations working without manual intervention:
  - **Config directory fallback** — if `~/.agitha/` doesn't exist but `~/.cyrus/` does, Agitha uses the legacy directory automatically. Rename the directory at your convenience.
  - **Legacy env var fold** — at boot, every `CYRUS_*` environment variable is copied into the matching `AGITHA_*` slot (only when the new name isn't already set). Both system-environment exports and entries in `~/.cyrus/.env` are picked up.
...
### Fixed
- **Pinned the Slack MCP server to v1.2.3.** v1.3.0 of `slack-mcp-server` requires additional Slack OAuth scopes; pinning avoids breaking existing Slack installations that haven't re-authorized. ([CYPACK-1239](https://linear.app/ceedar/issue/CYPACK-1239), [#1247](https://github.com/agithaagents/agitha/pull/1247))

### Packages

#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.57

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.57

#### agitha-claude-runner
- agitha-claude-runner@0.2.57

#### agitha-core
- agitha-core@0.2.57

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.57

#### agitha-codex-runner
- agitha-codex-runner@0.2.57

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.57

#### agitha-config-updater
- agitha-config-updater@0.2.57

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.57

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.57

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.57

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.57

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.57

#### agitha-edge-worker
- agitha-edge-worker@0.2.57
...
### Removed
- **Claude-in-Chrome browser integration is no longer enabled for agent sessions.** Agitha previously passed `--chrome` to every Claude runner session, which registered the `mcp__claude-in-chrome__*` MCP tools. In cloud-runtime sessions there is no path from the worker to the user's local Chrome extension, so those tools always failed with "Browser extension is not connected" — leading agents to loop on a misdiagnosis instead of falling back. The flag and the associated screenshot/GIF upload-guidance hooks have been removed. ([PRO-9](https://linear.app/ceedar/issue/PRO-9/failed-chrome-extension-not-connecting-oswaldo-okeefe), [#1244](https://github.com/agithaagents/agitha/pull/1244))

### Packages

#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.56

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.56

#### agitha-claude-runner
- agitha-claude-runner@0.2.56

#### agitha-core
- agitha-core@0.2.56

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.56

#### agitha-codex-runner
- agitha-codex-runner@0.2.56

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.56

#### agitha-config-updater
- agitha-config-updater@0.2.56

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.56

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.56

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.56

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.56

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.56

#### agitha-edge-worker
- agitha-edge-worker@0.2.56
...
### Fixed
- **`~/`-prefixed paths in `slackMcpConfigs`, `linearMcpConfigs`, and `githubMcpConfigs` are now expanded.** When agitha-hosted writes platform MCP config paths for a self-host runtime, it emits `~/.agitha/mcp-configs/...` strings that were passed verbatim to `fs.readFileSync`, which does not expand tildes — Claude sessions would crash with `ENOENT: ~/.agitha/mcp-configs/mcp-supabase.json`. EdgeWorker now runs the three platform MCP config arrays through `resolvePath` at construction and on config hot-reload, mirroring how repository-scoped paths have always been normalized. ([#1242](https://github.com/agithaagents/agitha/pull/1242))

### Packages

#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.55

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.55

#### agitha-claude-runner
- agitha-claude-runner@0.2.55

#### agitha-core
- agitha-core@0.2.55

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.55

#### agitha-codex-runner
- agitha-codex-runner@0.2.55

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.55

#### agitha-config-updater
- agitha-config-updater@0.2.55

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.55

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.55

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.55

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.55

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.55

#### agitha-edge-worker
- agitha-edge-worker@0.2.55
...
### Fixed
- **`log_failure_mode` MCP tool now registers when only `AGITHA_API_KEY` is set.** EdgeWorker previously required both `AGITHA_API_KEY` and `AGITHA_APP_URL` env vars to wire up the self-reported-failure-mode tool, so workspaces that hadn't overridden `AGITHA_APP_URL` silently shipped the failure-mode prompt addendum without a corresponding tool. The URL now falls back to the canonical default (`https://app.atagitha.com`) via the shared `getAgithaAppUrl()` helper, matching the remote session store. ([CYPACK-1232](https://linear.app/ceedar/issue/CYPACK-1232), [#1240](https://github.com/agithaagents/agitha/pull/1240))

### Packages

#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.54

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.54

#### agitha-claude-runner
- agitha-claude-runner@0.2.54

#### agitha-core
- agitha-core@0.2.54

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.54

#### agitha-codex-runner
- agitha-codex-runner@0.2.54

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.54

#### agitha-config-updater
- agitha-config-updater@0.2.54

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.54

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.54

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.54

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.54

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.54

#### agitha-edge-worker
- agitha-edge-worker@0.2.54
...
### Added
- **Per-platform allowed tools — explicit defaults exported from `agitha-core`** — `agitha-core` now exports three canonical lists (`LINEAR_DEFAULT_ALLOWED_TOOLS`, `SLACK_DEFAULT_ALLOWED_TOOLS`, `GITHUB_DEFAULT_ALLOWED_TOOLS`) plus a `getDefaultAllowedTools(platform)` resolver. Each list is **completely explicit** — workspace MCP prefixes (`mcp__linear`, `mcp__agitha-tools`, `mcp__agitha-docs`, and for Slack `mcp__slack`) are members of the list, not implicitly appended at runtime. agitha-hosted imports the same constants so the source of truth lives in one place. ([CYHOST-967](https://linear.app/ceedar/issue/CYHOST-967))
- **`EdgeConfig` accepts `slackAllowedTools` and `githubAllowedTools`** — two optional top-level keys for team-level platform overrides. When unset, the resolver falls back to the matching agitha-core default. ([CYHOST-967](https://linear.app/ceedar/issue/CYHOST-967))
- **`EdgeConfig` accepts `slackMcpConfigs`, `linearMcpConfigs`, `githubMcpConfigs`** — three optional arrays of filesystem paths to custom-integration `.mcp.json` files, one per surface. Slack chat sessions load the files in `slackMcpConfigs` directly (chat is repo-agnostic, no `repository.mcpConfigPath` lookup). Linear- and GitHub/GitLab-triggered issue sessions use the per-platform list only when the routed repo does NOT have its own `allowedTools` override; if the repo has its own allow-list, the agent uses `repository.mcpConfigPath` instead so the repo's permission rules and its server set always come from the same scope. Native MCP servers (Linear, Agitha tools, Agitha docs, and Slack when `SLACK_BOT_TOKEN` is set) are still spun up inline by the runtime — these lists govern custom integrations only. ([CYHOST-967](https://linear.app/ceedar/issue/CYHOST-967))
...
- **`ToolPermissionResolver` is now additive-only — no implicit MCP appending, no `SLACK_BOT_TOKEN`-conditional injection.** Previously `buildAllowedTools` and `buildChatAllowedTools` appended `mcp__linear`, `mcp__agitha-tools`, `mcp__agitha-docs` (and conditionally `mcp__slack` when `SLACK_BOT_TOKEN` was set in the process env) to every returned list. Now the explicit per-platform defaults include those prefixes verbatim, the resolver returns lists as-is, and `getWorkspaceMcpTools()` is removed. The `"readOnly"` preset now resolves to `SLACK_DEFAULT_ALLOWED_TOOLS` (the curated read-only set) instead of the bare `getReadOnlyTools()` list. ([CYHOST-967](https://linear.app/ceedar/issue/CYHOST-967))
...
- **Self-reported failure modes.** Every customer-facing agent session now has access to a new `mcp__agitha-tools__log_failure_mode` MCP tool and is instructed (via a shared system-prompt addendum appended to all Linear prompt flavors, the Slack entrypoint, and the GitHub entrypoint) to call it when the user expresses dissatisfaction or when it recognizes it has made 3+ unsuccessful attempts at the same problem. The tool POSTs to agitha-hosted, which opens (or comments on) a Linear ticket in the internal failure-modes project so the Agitha team can intervene before churn. Self-reporting is internal — users are not told about it. ([CYPACK-1226](https://linear.app/ceedar/issue/CYPACK-1226))

### Fixed
- **`slackAllowedTools`, `githubAllowedTools`, and the per-platform MCP config keys (`slackMcpConfigs`, `linearMcpConfigs`, `githubMcpConfigs`) are now honored after config hot-reload.** `ConfigManager.loadConfigSafely()` previously merged a hardcoded whitelist of fields from the parsed `config.json` and silently dropped every per-platform allow-list / MCP config key, so Slack and GitHub sessions kept resolving to the agitha-core defaults even when the workspace had a tighter override on disk. The merge and the change-detection list now include all six platform keys. ([CYHOST-967](https://linear.app/ceedar/issue/CYHOST-967))
- **Self-Managed GitLab MR replies** — `EdgeWorker` was instantiating `GitLabCommentService` with no `apiBaseUrl`, so every MR-reply request on a Self-Managed instance hit `gitlab.com` and 404'd. The base URL is now derived from the URL origin of the first configured repo with a `gitlabUrl`, so MR replies post against the correct host. Thanks [@tenforty](https://github.com/tenfourty) ([#1191](https://github.com/agithaagents/agitha/pull/1191))
- **Stop hook no longer blocks sessions for pre-existing untracked files** — Replaces the previous unconditional first-stop block (CYPACK-1204) with a more targeted git-aware guardrail. Agitha now scopes the end-of-session check to tracked changes and unpushed commits, ignoring stray untracked files (local scratch files, env files, IDE artifacts) outside `.gitignore`. New files Agitha creates via Write/Edit are still flagged via `git add --intent-to-add` if left uncommitted, so the "forgot to ship new work" check is preserved. ([CYPACK-1196](https://linear.app/ceedar/issue/CYPACK-1196), [#1204](https://github.com/agithaagents/agitha/pull/1204))

### Security
- **Patched 4 transitive dependency advisories** — Bumped `pnpm.overrides` for `brace-expansion` (≥5.0.6, DoS via large numeric ranges defeating `max` protection), `ws` (≥8.20.1, uninitialized memory disclosure on `close()` with `TypedArray` reason), `protobufjs` (≥7.5.8, DoS via unbounded recursive JSON descriptor expansion), and `uuid` (≥11.1.1, missing buffer bounds check in `v3`/`v5`/`v6`). `pnpm audit` now reports zero advisories. ([CYPACK-1230](https://linear.app/ceedar/issue/CYPACK-1230), [#1238](https://github.com/agithaagents/agitha/pull/1238))
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.53

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.53

#### agitha-claude-runner
- agitha-claude-runner@0.2.53

#### agitha-core
- agitha-core@0.2.53

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.53

#### agitha-codex-runner
- agitha-codex-runner@0.2.53

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.53

#### agitha-config-updater
- agitha-config-updater@0.2.53

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.53

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.53

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.53

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.53

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.53

#### agitha-edge-worker
- agitha-edge-worker@0.2.53
...
### Added
- **Agitha-authored PRs and MRs are now reliably tagged** — After every `gh pr create`/`gh pr edit`, `glab mr create`/`glab mr update`/`glab mr edit`, or `gt submit` command, Agitha automatically appends a hidden `<!-- generated-by-agitha -->` marker to the live PR/MR description if it isn't already there. This ensures the GitHub/GitLab webhook handlers can recognize Agitha-authored PRs (so "Changes requested" events get forwarded back) even when the agent forgets to include the marker in the body it submits. ([CYPACK-1141](https://linear.app/ceedar/issue/CYPACK-1141), [#1162](https://github.com/agithaagents/agitha/pull/1162))
...
- **Warm Claude sessions are now opt-in** — On startup, Agitha no longer pre-spawns Claude Code subprocesses for the 30 most recent sessions by default. To restore the previous near-zero cold-start latency on the first message after a restart, set `AGITHA_ENABLE_WARM_SESSIONS=1` in the environment. ([CYPACK-1116](https://linear.app/ceedar/issue/CYPACK-1116))
- **Claude SDK subprocesses now exit at turn end unless warm mode is enabled** — When `AGITHA_ENABLE_WARM_SESSIONS` is unset, the streaming prompt is completed when the SDK emits a `result` message, which lets the underlying Claude Code subprocess actually exit and free its memory at the end of a turn (restores the pre-warm-sessions behavior). When `AGITHA_ENABLE_WARM_SESSIONS=1`, the streaming prompt stays open and the subprocess is kept alive so follow-up messages reuse the warm session.
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.50

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.50

#### agitha-claude-runner
- agitha-claude-runner@0.2.50

#### agitha-core
- agitha-core@0.2.50

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.50

#### agitha-codex-runner
- agitha-codex-runner@0.2.50

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.50

#### agitha-config-updater
- agitha-config-updater@0.2.50

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.50

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.50

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.50

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.50

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.50

#### agitha-edge-worker
- agitha-edge-worker@0.2.50
...
### Fixed
- **Claude sessions inherit the parent process environment again** — `@anthropic-ai/claude-agent-sdk` v0.2.113 reverted to no longer overlaying `process.env` onto the env passed to spawned sessions, which left Agitha-launched Claude processes without `HOME` (and other inherited vars). That broke GPG-signed commits, `gh` CLI authentication, and any other tool that relies on a real home directory or the user's shell environment. Agitha now spreads `process.env` explicitly when invoking the SDK so these tools work as expected. ([#1150](https://github.com/agithaagents/agitha/pull/1150))

### Packages

#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.49

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.49

#### agitha-claude-runner
- agitha-claude-runner@0.2.49

#### agitha-core
- agitha-core@0.2.49

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.49

#### agitha-codex-runner
- agitha-codex-runner@0.2.49

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.49

#### agitha-config-updater
- agitha-config-updater@0.2.49

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.49

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.49

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.49

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.49

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.49

#### agitha-edge-worker
- agitha-edge-worker@0.2.49
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.48

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.48

#### agitha-claude-runner
- agitha-claude-runner@0.2.48

#### agitha-core
- agitha-core@0.2.48

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.48

#### agitha-codex-runner
- agitha-codex-runner@0.2.48

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.48

#### agitha-config-updater
- agitha-config-updater@0.2.48

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.48

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.48

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.48

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.48

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.48

#### agitha-edge-worker
- agitha-edge-worker@0.2.48
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.47

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.47

#### agitha-claude-runner
- agitha-claude-runner@0.2.47

#### agitha-core
- agitha-core@0.2.47

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.47

#### agitha-codex-runner
- agitha-codex-runner@0.2.47

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.47

#### agitha-config-updater
- agitha-config-updater@0.2.47

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.47

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.47

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.47

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.47

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.47

#### agitha-edge-worker
- agitha-edge-worker@0.2.47
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.46

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.46

#### agitha-claude-runner
- agitha-claude-runner@0.2.46

#### agitha-core
- agitha-core@0.2.46

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.46

#### agitha-codex-runner
- agitha-codex-runner@0.2.46

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.46

#### agitha-config-updater
- agitha-config-updater@0.2.46

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.46

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.46

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.46

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.46

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.46

#### agitha-edge-worker
- agitha-edge-worker@0.2.46
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.45

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.45

#### agitha-claude-runner
- agitha-claude-runner@0.2.45

#### agitha-core
- agitha-core@0.2.45

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.45

#### agitha-codex-runner
- agitha-codex-runner@0.2.45

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.45

#### agitha-config-updater
- agitha-config-updater@0.2.45

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.45

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.45

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.45

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.45

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.45

#### agitha-edge-worker
- agitha-edge-worker@0.2.45
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.44

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.44

#### agitha-claude-runner
- agitha-claude-runner@0.2.44

#### agitha-core
- agitha-core@0.2.44

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.44

#### agitha-codex-runner
- agitha-codex-runner@0.2.44

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.44

#### agitha-config-updater
- agitha-config-updater@0.2.44

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.44

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.44

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.44

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.44

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.44

#### agitha-edge-worker
- agitha-edge-worker@0.2.44
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.43

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.43

#### agitha-claude-runner
- agitha-claude-runner@0.2.43

#### agitha-core
- agitha-core@0.2.43

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.43

#### agitha-codex-runner
- agitha-codex-runner@0.2.43

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.43

#### agitha-config-updater
- agitha-config-updater@0.2.43

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.43

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.43

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.43

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.43

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.43

#### agitha-edge-worker
- agitha-edge-worker@0.2.43
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.42

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.42

#### agitha-claude-runner
- agitha-claude-runner@0.2.42

#### agitha-core
- agitha-core@0.2.42

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.42

#### agitha-codex-runner
- agitha-codex-runner@0.2.42

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.42

#### agitha-config-updater
- agitha-config-updater@0.2.42

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.42

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.42

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.42

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.42

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.42

#### agitha-edge-worker
- agitha-edge-worker@0.2.42
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.41

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.41

#### agitha-claude-runner
- agitha-claude-runner@0.2.41

#### agitha-core
- agitha-core@0.2.41

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.41

#### agitha-codex-runner
- agitha-codex-runner@0.2.41

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.41

#### agitha-config-updater
- agitha-config-updater@0.2.41

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.41

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.41

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.41

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.41

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.41

#### agitha-edge-worker
- agitha-edge-worker@0.2.41
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.40

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.40

#### agitha-claude-runner
- agitha-claude-runner@0.2.40

#### agitha-core
- agitha-core@0.2.40

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.40

#### agitha-codex-runner
- agitha-codex-runner@0.2.40

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.40

#### agitha-config-updater
- agitha-config-updater@0.2.40

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.40

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.40

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.40

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.40

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.40

#### agitha-edge-worker
- agitha-edge-worker@0.2.40
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.39

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.39

#### agitha-claude-runner
- agitha-claude-runner@0.2.39

#### agitha-core
- agitha-core@0.2.39

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.39

#### agitha-codex-runner
- agitha-codex-runner@0.2.39

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.39

#### agitha-config-updater
- agitha-config-updater@0.2.39

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.39

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.39

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.39

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.39

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.39

#### agitha-edge-worker
- agitha-edge-worker@0.2.39
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.38

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.38

#### agitha-claude-runner
- agitha-claude-runner@0.2.38

#### agitha-core
- agitha-core@0.2.38

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.38

#### agitha-codex-runner
- agitha-codex-runner@0.2.38

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.38

#### agitha-config-updater
- agitha-config-updater@0.2.38

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.38

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.38

#### agitha-gitlab-event-transport
- agitha-gitlab-event-transport@0.2.38

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.38

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.38

#### agitha-edge-worker
- agitha-edge-worker@0.2.38
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.37

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.37

#### agitha-claude-runner
- agitha-claude-runner@0.2.37

#### agitha-core
- agitha-core@0.2.37

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.37

#### agitha-codex-runner
- agitha-codex-runner@0.2.37

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.37

#### agitha-config-updater
- agitha-config-updater@0.2.37

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.37

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.37

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.37

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.37

#### agitha-edge-worker
- agitha-edge-worker@0.2.37
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.36

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.36

#### agitha-claude-runner
- agitha-claude-runner@0.2.36

#### agitha-core
- agitha-core@0.2.36

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.36

#### agitha-codex-runner
- agitha-codex-runner@0.2.36

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.36

#### agitha-config-updater
- agitha-config-updater@0.2.36

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.36

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.36

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.36

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.36

#### agitha-edge-worker
- agitha-edge-worker@0.2.36
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.35

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.35

#### agitha-claude-runner
- agitha-claude-runner@0.2.35

#### agitha-core
- agitha-core@0.2.35

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.35

#### agitha-codex-runner
- agitha-codex-runner@0.2.35

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.35

#### agitha-config-updater
- agitha-config-updater@0.2.35

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.35

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.35

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.35

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.35

#### agitha-edge-worker
- agitha-edge-worker@0.2.35
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.34

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.34

#### agitha-claude-runner
- agitha-claude-runner@0.2.34

#### agitha-core
- agitha-core@0.2.34

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.34

#### agitha-codex-runner
- agitha-codex-runner@0.2.34

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.34

#### agitha-config-updater
- agitha-config-updater@0.2.34

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.34

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.34

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.34

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.34

#### agitha-edge-worker
- agitha-edge-worker@0.2.34
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.33

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.33

#### agitha-claude-runner
- agitha-claude-runner@0.2.33

#### agitha-core
- agitha-core@0.2.33

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.33

#### agitha-codex-runner
- agitha-codex-runner@0.2.33

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.33

#### agitha-config-updater
- agitha-config-updater@0.2.33

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.33

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.33

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.33

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.33

#### agitha-edge-worker
- agitha-edge-worker@0.2.33
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.32

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.32

#### agitha-claude-runner
- agitha-claude-runner@0.2.32

#### agitha-core
- agitha-core@0.2.32

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.32

#### agitha-codex-runner
- agitha-codex-runner@0.2.32

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.32

#### agitha-config-updater
- agitha-config-updater@0.2.32

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.32

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.32

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.32

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.32

#### agitha-edge-worker
- agitha-edge-worker@0.2.32
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.31

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.31

#### agitha-claude-runner
- agitha-claude-runner@0.2.31

#### agitha-core
- agitha-core@0.2.31

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.31

#### agitha-codex-runner
- agitha-codex-runner@0.2.31

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.31

#### agitha-config-updater
- agitha-config-updater@0.2.31

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.31

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.31

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.31

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.31

#### agitha-edge-worker
- agitha-edge-worker@0.2.31
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.30

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.30

#### agitha-claude-runner
- agitha-claude-runner@0.2.30

#### agitha-core
- agitha-core@0.2.30

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.30

#### agitha-codex-runner
- agitha-codex-runner@0.2.30

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.30

#### agitha-config-updater
- agitha-config-updater@0.2.30

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.30

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.30

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.30

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.30

#### agitha-edge-worker
- agitha-edge-worker@0.2.30
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.29

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.29

#### agitha-claude-runner
- agitha-claude-runner@0.2.29

#### agitha-core
- agitha-core@0.2.29

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.29

#### agitha-codex-runner
- agitha-codex-runner@0.2.29

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.29

#### agitha-config-updater
- agitha-config-updater@0.2.29

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.29

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.29

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.29

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.29

#### agitha-edge-worker
- agitha-edge-worker@0.2.29
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.28

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.28

#### agitha-claude-runner
- agitha-claude-runner@0.2.28

#### agitha-core
- agitha-core@0.2.28

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.28

#### agitha-codex-runner
- agitha-codex-runner@0.2.28

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.28

#### agitha-config-updater
- agitha-config-updater@0.2.28

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.28

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.28

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.28

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.28

#### agitha-edge-worker
- agitha-edge-worker@0.2.28
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.27

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.27

#### agitha-claude-runner
- agitha-claude-runner@0.2.27

#### agitha-core
- agitha-core@0.2.27

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.27

#### agitha-codex-runner
- agitha-codex-runner@0.2.27

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.27

#### agitha-config-updater
- agitha-config-updater@0.2.27

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.27

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.27

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.27

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.27

#### agitha-edge-worker
- agitha-edge-worker@0.2.27
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.26

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.26

#### agitha-claude-runner
- agitha-claude-runner@0.2.26

#### agitha-core
- agitha-core@0.2.26

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.26

#### agitha-codex-runner
- agitha-codex-runner@0.2.26

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.26

#### agitha-config-updater
- agitha-config-updater@0.2.26

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.26

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.26

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.26

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.26

#### agitha-edge-worker
- agitha-edge-worker@0.2.26
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.25

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.25

#### agitha-claude-runner
- agitha-claude-runner@0.2.25

#### agitha-core
- agitha-core@0.2.25

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.25

#### agitha-codex-runner
- agitha-codex-runner@0.2.25

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.25

#### agitha-config-updater
- agitha-config-updater@0.2.25

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.25

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.25

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.25

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.25

#### agitha-edge-worker
- agitha-edge-worker@0.2.25
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.24

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.24

#### agitha-claude-runner
- agitha-claude-runner@0.2.24

#### agitha-core
- agitha-core@0.2.24

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.24

#### agitha-codex-runner
- agitha-codex-runner@0.2.24

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.24

#### agitha-config-updater
- agitha-config-updater@0.2.24

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.24

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.24

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.24

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.24

#### agitha-edge-worker
- agitha-edge-worker@0.2.24
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.23

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.23

#### agitha-claude-runner
- agitha-claude-runner@0.2.23

#### agitha-core
- agitha-core@0.2.23

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.23

#### agitha-codex-runner
- agitha-codex-runner@0.2.23

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.23

#### agitha-config-updater
- agitha-config-updater@0.2.23

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.23

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.23

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.23

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.23

#### agitha-edge-worker
- agitha-edge-worker@0.2.23
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.22

#### agitha-mcp-tools
- agitha-mcp-tools@0.2.22

#### agitha-claude-runner
- agitha-claude-runner@0.2.22

#### agitha-core
- agitha-core@0.2.22

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.22

#### agitha-codex-runner
- agitha-codex-runner@0.2.22

#### agitha-cursor-runner
- agitha-cursor-runner@0.2.22

#### agitha-config-updater
- agitha-config-updater@0.2.22

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.22

#### agitha-github-event-transport
- agitha-github-event-transport@0.2.22

#### agitha-slack-event-transport
- agitha-slack-event-transport@0.2.22

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.22

#### agitha-edge-worker
- agitha-edge-worker@0.2.22
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.21

#### agitha-config-updater
- agitha-config-updater@0.2.21

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.21

#### agitha-claude-runner
- agitha-claude-runner@0.2.21

#### agitha-core
- agitha-core@0.2.21

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.21

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.21

#### agitha-edge-worker
- agitha-edge-worker@0.2.21
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.20

#### agitha-config-updater
- agitha-config-updater@0.2.20

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.20

#### agitha-claude-runner
- agitha-claude-runner@0.2.20

#### agitha-core
- agitha-core@0.2.20

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.20

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.20

#### agitha-edge-worker
- agitha-edge-worker@0.2.20
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.19

#### agitha-config-updater
- agitha-config-updater@0.2.19

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.19

#### agitha-claude-runner
- agitha-claude-runner@0.2.19

#### agitha-core
- agitha-core@0.2.19

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.19

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.19

#### agitha-edge-worker
- agitha-edge-worker@0.2.19
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.18

#### agitha-config-updater
- agitha-config-updater@0.2.18

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.18

#### agitha-claude-runner
- agitha-claude-runner@0.2.18

#### agitha-core
- agitha-core@0.2.18

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.18

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.18

#### agitha-edge-worker
- agitha-edge-worker@0.2.18
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.17

#### agitha-config-updater
- agitha-config-updater@0.2.17

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.17

#### agitha-claude-runner
- agitha-claude-runner@0.2.17

#### agitha-core
- agitha-core@0.2.17

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.17

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.17

#### agitha-edge-worker
- agitha-edge-worker@0.2.17
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.16

#### agitha-config-updater
- agitha-config-updater@0.2.16

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.16

#### agitha-claude-runner
- agitha-claude-runner@0.2.16

#### agitha-core
- agitha-core@0.2.16

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.16

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.16

#### agitha-edge-worker
- agitha-edge-worker@0.2.16
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.15

#### agitha-config-updater
- agitha-config-updater@0.2.15

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.15

#### agitha-claude-runner
- agitha-claude-runner@0.2.15

#### agitha-core
- agitha-core@0.2.15

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.15

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.15

#### agitha-edge-worker
- agitha-edge-worker@0.2.15
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.14

#### agitha-config-updater
- agitha-config-updater@0.2.14

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.14

#### agitha-claude-runner
- agitha-claude-runner@0.2.14

#### agitha-core
- agitha-core@0.2.14

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.14

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.14

#### agitha-edge-worker
- agitha-edge-worker@0.2.14
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.13

#### agitha-config-updater
- agitha-config-updater@0.2.13

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.13

#### agitha-claude-runner
- agitha-claude-runner@0.2.13

#### agitha-core
- agitha-core@0.2.13

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.13

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.13

#### agitha-edge-worker
- agitha-edge-worker@0.2.13
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.12

#### agitha-config-updater
- agitha-config-updater@0.2.12

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.12

#### agitha-claude-runner
- agitha-claude-runner@0.2.12

#### agitha-core
- agitha-core@0.2.12

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.12

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.12

#### agitha-edge-worker
- agitha-edge-worker@0.2.12
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.11

#### agitha-config-updater
- agitha-config-updater@0.2.11

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.11

#### agitha-claude-runner
- agitha-claude-runner@0.2.11

#### agitha-core
- agitha-core@0.2.11

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.11

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.11

#### agitha-edge-worker
- agitha-edge-worker@0.2.11
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.10

#### agitha-config-updater
- agitha-config-updater@0.2.10

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.10

#### agitha-claude-runner
- agitha-claude-runner@0.2.10

#### agitha-core
- agitha-core@0.2.10

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.10

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.10

#### agitha-edge-worker
- agitha-edge-worker@0.2.10
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.9

#### agitha-config-updater
- agitha-config-updater@0.2.9

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.9

#### agitha-claude-runner
- agitha-claude-runner@0.2.9

#### agitha-core
- agitha-core@0.2.9

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.9

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.9

#### agitha-edge-worker
- agitha-edge-worker@0.2.9
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.8

#### agitha-config-updater
- agitha-config-updater@0.2.8

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.8

#### agitha-claude-runner
- agitha-claude-runner@0.2.8

#### agitha-core
- agitha-core@0.2.8

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.8

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.8

#### agitha-edge-worker
- agitha-edge-worker@0.2.8
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.7

#### agitha-config-updater
- agitha-config-updater@0.2.7

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.7

#### agitha-claude-runner
- agitha-claude-runner@0.2.7

#### agitha-core
- agitha-core@0.2.7

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.7

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.7

#### agitha-edge-worker
- agitha-edge-worker@0.2.7
...
### Changed
- **Default model upgraded to Opus** - Agitha now uses Claude Opus as the default model with Sonnet as fallback (previously Sonnet with Haiku fallback). This provides higher quality responses for all tasks. ([CYPACK-613](https://linear.app/ceedar/issue/CYPACK-613))
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.6

#### agitha-config-updater
- agitha-config-updater@0.2.6

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.6

#### agitha-claude-runner
- agitha-claude-runner@0.2.6

#### agitha-core
- agitha-core@0.2.6

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.6

#### agitha-gemini-runner
- agitha-gemini-runner@0.2.6

#### agitha-edge-worker
- agitha-edge-worker@0.2.6
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.5

#### agitha-config-updater
- agitha-config-updater@0.2.5

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.5

#### agitha-claude-runner
- agitha-claude-runner@0.2.5

#### agitha-core
- agitha-core@0.2.5

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.5

#### agitha-edge-worker
- agitha-edge-worker@0.2.5
...
### Added
- **Google Gemini AI support** - Agitha now supports Google's Gemini models alongside Claude. Choose which AI processes your issues by adding labels to Linear issues: use `gemini`, `gemini-2.5-pro`, `gemini-2.5-flash`, or `gemini-3-pro` for Gemini models, or `claude`, `sonnet`, or `opus` for Claude models. If no AI label is present, Agitha defaults to Claude. This gives you flexibility to select the best AI for each task.
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.4

#### agitha-config-updater
- agitha-config-updater@0.2.4

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.4

#### agitha-claude-runner
- agitha-claude-runner@0.2.4

#### agitha-core
- agitha-core@0.2.4

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.4

#### agitha-edge-worker
- agitha-edge-worker@0.2.4
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.3

#### agitha-config-updater
- agitha-config-updater@0.2.3

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.3

#### agitha-claude-runner
- agitha-claude-runner@0.2.3

#### agitha-core
- agitha-core@0.2.3

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.3

#### agitha-edge-worker
- agitha-edge-worker@0.2.3
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.2

#### agitha-config-updater
- agitha-config-updater@0.2.2

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.2

#### agitha-claude-runner
- agitha-claude-runner@0.2.2

#### agitha-core
- agitha-core@0.2.2

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.2

#### agitha-edge-worker
- agitha-edge-worker@0.2.2
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.1

#### agitha-config-updater
- agitha-config-updater@0.2.1

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.1

#### agitha-claude-runner
- agitha-claude-runner@0.2.1

#### agitha-core
- agitha-core@0.2.1

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.1

#### agitha-edge-worker
- agitha-edge-worker@0.2.1
...
### Added
- **Cloudflare Tunnel Transport Client**: New `agitha-cloudflare-tunnel-client` package for receiving configuration updates and webhooks from agitha-hosted
  - Uses Cloudflare tunnels via `cloudflared` npm package for secure communication
  - Validates customer subscriptions with agitha-hosted API
  - Handles configuration updates (repositories, environment variables, MCP servers)
  - Receives Linear webhook payloads forwarded through agitha-hosted
  - Repository management (automatically clones/verifies repositories to `~/.agitha/repos/<repo-name>`)
  - All file operations restricted to `~/.agitha` directory for security
  - Will replace `ndjson-client` for customers using agitha-hosted service
- **Setup Waiting Mode**: After running `agitha auth`, the client now enters a waiting state to receive configuration from the server
...
### Fixed
- Agitha client now stays running when all repositories are removed after onboarding, allowing it to receive new configuration from app.atagitha.com
...
- Added missing `routingLabels` and `projectKeys` fields to `AgithaConfigPayload` type in config-updater package
- Config handler now properly processes and saves label routing and project routing parameters when received from agitha-hosted
...
### Changed
- **Linear Event Transport**: Refactored `agitha-linear-webhook-client` to `agitha-linear-event-transport` for simplified webhook handling
...
#### agitha-cloudflare-tunnel-client
- agitha-cloudflare-tunnel-client@0.2.0

#### agitha-config-updater
- agitha-config-updater@0.2.0

#### agitha-linear-event-transport
- agitha-linear-event-transport@0.2.0

#### agitha-claude-runner
- agitha-claude-runner@0.2.0

#### agitha-core
- agitha-core@0.2.0

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.2.0

#### agitha-edge-worker
- agitha-edge-worker@0.2.0
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.32

#### agitha-edge-worker
- agitha-edge-worker@0.0.41

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.25

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.0.4
...
#### agitha-edge-worker
- agitha-edge-worker@0.0.40

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.0.3
...
- Orchestrator agents are explicitly instructed not to assign themselves (Agitha) as a delegate when creating sub-issues
...
### Fixed
- Fixed missing `agitha-simple-agent-runner` package publication that broke installation of agitha-ai@0.1.56

### Packages

#### agitha-simple-agent-runner
- agitha-simple-agent-runner@0.0.2

#### agitha-edge-worker
- agitha-edge-worker@0.0.39
...
- **Simple agent runner package**: Added new `agitha-simple-agent-runner` package for constrained agent queries that return one of a predefined set of responses (e.g., "yes", "no"). Features type-safe enumerated responses, comprehensive error handling, and progress tracking.
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.31

#### agitha-core
- agitha-core@0.0.19

#### agitha-edge-worker
- agitha-edge-worker@0.0.38

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.24
...
### Added
- **Dynamic configuration updates**: Agitha now automatically detects and applies changes to `~/.agitha/config.json` without requiring a restart
  - Add or remove repositories on the fly while Agitha continues running
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.30

#### agitha-core
- agitha-core@0.0.18

#### agitha-edge-worker
- agitha-edge-worker@0.0.37

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.23
...
### Added
- **Automatic MCP config detection**: Agitha now automatically detects and loads `.mcp.json` files in the repository root. The `.mcp.json` serves as a base configuration that can be extended by explicit `mcpConfigPath` settings, allowing for composable MCP server configurations.

### Fixed
- **Custom instructions now work correctly**: Fixed critical bug where `appendSystemPrompt` was being silently ignored, causing Agitha to not follow custom instructions or agent guidance. The feature has been fixed to use the correct SDK API (`systemPrompt.append`), making custom prompts and Linear agent guidance work as intended.

### Packages

#### agitha-claude-runner
- agitha-claude-runner@0.0.29

#### agitha-core
- agitha-core@0.0.17

#### agitha-edge-worker
- agitha-edge-worker@0.0.36

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.22
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.28

#### agitha-core
- agitha-core@0.0.16

#### agitha-edge-worker
- agitha-edge-worker@0.0.35

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.21
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.27

#### agitha-core
- agitha-core@0.0.15

#### agitha-edge-worker
- agitha-edge-worker@0.0.34

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.20
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.26

#### agitha-core
- agitha-core@0.0.14

#### agitha-edge-worker
- agitha-edge-worker@0.0.33

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.19
...
- **Global setup script support**: Added `global_setup_script` optional field in config.json
  - Runs before repository-specific `agitha-setup.sh` when creating git worktrees
...
#### agitha-linear-webhook-client
- agitha-linear-webhook-client@0.0.3
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.25

#### agitha-core
- agitha-core@0.0.13

#### agitha-edge-worker
- agitha-edge-worker@0.0.32
...
#### agitha-linear-webhook-client
- agitha-linear-webhook-client@0.0.2

#### agitha-edge-worker
- agitha-edge-worker@0.0.30
...
- **@agitha /label-based-prompt command**: New special command for mention-triggered sessions
  - Use `@agitha /label-based-prompt` in comments to trigger label-based prompts instead of mention prompts
  - Automatically determines and includes appropriate system prompts based on issue labels
  - Maintains full backwards compatibility with regular `@agitha` mentions
...
- Replaced external agitha-mcp-tools MCP server with inline tools using SDK callbacks for better performance
- Agitha tools (file upload, agent session creation, feedback) now run in-process instead of via separate MCP server
- Enhanced orchestrator prompt to explicitly require reading/viewing all screenshots taken for visual verification

### Removed
- Removed agitha-mcp-tools package in favor of inline tool implementation

## [0.1.45] - 2025-08-28

### Added
- New `agitha-mcp-tools` package providing MCP tools for Linear integration
  ...
  - Automatically available in all Agitha sessions without additional configuration
...
#### agitha-mcp-tools
- agitha-mcp-tools@0.3.0 - Already published (not part of this release)

#### agitha-core
- agitha-core@0.0.11

#### agitha-claude-runner
- agitha-claude-runner@0.0.23

#### agitha-edge-worker
- agitha-edge-worker@0.0.28

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.17
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.22

#### agitha-edge-worker
- agitha-edge-worker@0.0.27
...
- Model configuration support for Claude Pro users
  ...
  - Environment variables: `AGITHA_DEFAULT_MODEL` and `AGITHA_DEFAULT_FALLBACK_MODEL`
  - Global config: `defaultModel` and `defaultFallbackModel` in `~/.agitha/config.json`
...
#### agitha-claude-runner  
- agitha-claude-runner@0.0.21

#### agitha-edge-worker
- agitha-edge-worker@0.0.26
...
#### agitha-claude-runner  
- agitha-claude-runner@0.0.20

#### agitha-edge-worker
- agitha-edge-worker@0.0.25

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.16
...
- Resolved issue where Agitha would fail to respond when it was initially delegated when the receiver was down
...
#### agitha-claude-runner  
- agitha-claude-runner@0.0.19

#### agitha-edge-worker
- agitha-edge-worker@0.0.24
...
### Added
- Customer subscription validation for Agitha Pro users
  ...
  - Validates subscription when setting customer ID via `agitha set-customer-id` command
...
- Updated Linear SDK from v54 to v55.1.0 to support Agent Activity Signals
  - Stop button in Linear UI now sends a deterministic `stop` signal that Agitha responds to immediately
  - When you click the stop button while Agitha is working, it will cleanly halt all operations and confirm the stop action
...
#### agitha-claude-runner  
- agitha-claude-runner@0.0.18

#### agitha-edge-worker
- agitha-edge-worker@0.0.23

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.15
...
- Simplified initial setup by removing configuration prompts for MCP, labels, Linear teams, allowed tools, and workspace directory
  ...
  - Workspace directory automatically uses `~/.agitha/workspaces/<repo-name>`
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.17

#### agitha-edge-worker
- agitha-edge-worker@0.0.22
...
- Native Linear attachments (like Sentry error links) are now included in the issue context sent to Claude
  - Agitha now fetches attachments using Linear's native attachment API
...
- New command **`agitha add-repository`** - Add a new repository configuration, thanks new contributor @Maxim-Filimonov !
- Attachment support for comments - Agitha now downloads and provides access to attachments added in Linear comments
  - Attachments are automatically downloaded when users post comments with URLs or files
  - Downloaded to `~/.agitha/<workspace>/attachments` directory
...
- Subscription management built right into the CLI (because who wants another dashboard?)
  - `agitha billing` - Opens your Stripe portal to manage subscription, payment methods, and download invoices
  - `agitha set-customer-id` - Saves your customer ID after signup (copy-paste friendly)
...
- Fixed issue where messages from @ Agitha mention comments weren't being added to context
- Fixed issue where sub-issue base branches weren't being added to the user-prompt template, causing Agitha to create PRs against the default branch instead
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.16

#### agitha-edge-worker
- agitha-edge-worker@0.0.21
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.15

#### agitha-edge-worker
- agitha-edge-worker@0.0.20
...
### Added
- Instant response is now sent when receiving follow-up messages in an existing conversation, providing immediate feedback that Agitha is working on the request
  - Shows "I've queued up your message as guidance" when Agitha is still processing a previous request
  - Shows "Getting started on that..." when Agitha is ready to process the new request immediately
...
- Model notification at thread initialization - Agitha now announces which Claude model is being used (e.g., "Using model: claude-3-opus-20240229") when starting work on an issue
- Task tool execution markers in Linear comments - Agitha now clearly indicates when automated Task tools are running
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.14

#### agitha-edge-worker
- agitha-edge-worker@0.0.19

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.14
...
### Added
- Instant acknowledgment responses when Agitha receives a request, providing immediate feedback to users
- Role mode notifications when issue labels trigger specific workflows (e.g., "Entering 'debugger' mode because of the 'Bug' label")
- You can now append custom instructions to Claude's system prompt via `appendInstruction` in repository config (~/.agitha/config.json) - because sometimes Claude needs a gentle reminder that your variable names are art, not accidents
...
#### agitha-core
- agitha-core@0.0.6-alpha.0

#### agitha-claude-runner
- agitha-claude-runner@0.0.13-alpha.0

#### agitha-edge-worker
- agitha-edge-worker@0.0.18-alpha.0

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.13-alpha.0

#### agitha-ai (CLI)
- agitha-ai@0.1.35-alpha.0
...
### Fixed
- Made conversation history of threads be resumable after Agitha restarts
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.13

#### agitha-edge-worker
- agitha-edge-worker@0.0.18

#### agitha-ndjson-client
- agitha-ndjson-client@0.0.13
...
- Automatic ngrok tunnel setup for external access
  ...
  - Agitha will ask for your ngrok auth token on first run and handle the rest
...
- Webhook debugging via `AGITHA_WEBHOOK_DEBUG=true` environment variable - see exactly what Linear is (or isn't) sending you
...
### Added
- Added `agitha check-tokens` command to validate all Linear OAuth tokens across repositories
- Added `agitha refresh-token` command with OAuth flow integration to renew expired tokens
...
### Changed
- Configuration file location moved from `.edge-config.json` in current directory to `~/.agitha/config.json`
...
- Default workspace directory changed from `{repository}/workspaces` to `~/.agitha/workspaces/{repo-name}`
  - Centralizes all agitha-related files in the user's home directory
...
### Fixed
- Fixed webhook signature verification failures after restarting agitha by extending edge worker registration TTL from 1 hour to 90 days
  - Resolves "Webhook signature verification failed for all registered handlers" error that occurred when agitha was stopped and restarted
...
#### agitha-claude-runner
- agitha-claude-runner@0.0.8

#### agitha-edge-worker
- agitha-edge-worker@0.0.10
...
### Changed
- **BREAKING**: Renamed `AGITHA_WEBHOOK_BASE_URL` to `AGITHA_BASE_URL` for clearer naming
  - **Action Required**: Update environment configuration to use `AGITHA_BASE_URL` instead of `AGITHA_WEBHOOK_BASE_URL`
  - **Legacy Support**: `AGITHA_WEBHOOK_BASE_URL` is still supported for backward compatibility but deprecated
  - The variable serves both webhook and OAuth callback purposes since they run on the same server

### Packages

#### agitha-edge-worker
- agitha-edge-worker@0.0.9
...
- Renamed repository setup script from `secretagentsetup.sh` to `agitha-setup.sh`
...
### Packages
- agitha-claude-runner@0.0.3
- agitha-core@0.0.3
- agitha-edge-worker@0.0.3
- agitha-ndjson-client@0.0.3

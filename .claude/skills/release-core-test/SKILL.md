...
description: Invoke when dev-testing a Agitha change that spans CYPACK (edgeworker + CLI) and CYHOST (Vercel-hosted GUI) and the hosted GUI needs to point at an unreleased `agitha-core` from this repo. Publishes `agitha-core` (and `claude-runner` if needed) as a `-test.N` prerelease under the npm `test` dist-tag so CYHOST can install it via `agitha-core@test` without affecting the `latest` dist-tag or shipping a real release.
---

# Release Core (Test)

Publish ONLY the `agitha-core` package (and `claude-runner` if needed) as a dev/test prerelease to npm under the `test` dist-tag. This is a developer convenience for iterating on `agitha-core` without going through the full release flow.
...
It also does NOT publish any other workspace package (no `edge-worker`, no `agitha` CLI, no `linear-event-transport`, etc.). The only exception is `claude-runner`, which is republished if and only if its `workspace:*` reference would otherwise resolve to an unpublished version (see step 3 below).
...
`agitha-core` depends on `claude-runner` via `workspace:*`. When `agitha-core` is published, that `workspace:*` is rewritten to the version currently in `packages/claude-runner/package.json`. If that version isn't actually on npm yet (because `claude-runner` has local changes since its last publish), consumers installing `agitha-core@test` will fail to resolve the dependency.
...
npm view "agitha-claude-runner@${LOCAL_CR}" version 2>/dev/null
...
```bash
# Determine the next -test.N for claude-runner (same algorithm as agitha-core below)
...
cd ../..
pnpm install   # refresh lockfile so agitha-core picks up the new claude-runner
```

Otherwise, skip this step entirely.

### 4. Bump `agitha-core` to the next `-test.N` prerelease version
...
npm view agitha-core versions --json \
  | jq -r '.[]' \
...
### 5. Publish `agitha-core` with the `test` dist-tag
...
- `npm install agitha-core@test` will install this version.
- The `latest` dist-tag is **not** moved, so production installs (`npm install agitha-core`) are unaffected.

### 6. Verify the dist-tags landed correctly

```bash
npm view agitha-core dist-tags
```
...
```
Published agitha-core@<version> under dist-tag `test`.

Install with:
  npm install agitha-core@test
or pinned:
  npm install agitha-core@<version>
```
...
- Prerelease versions (anything with a `-` suffix like `-test.0`) are never auto-installed by `npm install agitha-core` without an explicit tag or version spec, so this is safe to run repeatedly.
- Run `pnpm install` after publishing `claude-runner` (if applicable) so the lockfile reflects the new published version before `agitha-core` is built/published.
- `pnpm install` + `pnpm build` from root takes ~3 minutes combined on a clean tree — plan command timeouts accordingly.

## Examples

- "release core test" - Run a dev release of agitha-core
